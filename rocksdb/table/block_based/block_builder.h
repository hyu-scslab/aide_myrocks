...
class BlockBuilder {
 public:
...
#ifdef VERSION_INDEX
  // Just like Reset(), but for version index.
  void ViReset();

  // Just like SwapAndReset(), but for version index.
  void ViSwapAndReset(std::string& buffer);

  size_t EstimateSizeOfVersionIndex() const;
#endif
  ...
  void Add(const Slice& key, const Slice& value,
           ...
           const uint32_t vi_offset = 0,
           const int nullbytes = 0);
#else
...
#ifdef VERSION_INDEX
  std::string vi_buffer_;           // Destination buffer for version index
#endif
