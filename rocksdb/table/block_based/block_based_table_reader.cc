...
Status ReadBlockFromFile(
  ...
  // fprintf(stderr, "[SMARTSSD] ReadBlockFromFile\n");
  ...
  if (async_read && prefetch_buffer != nullptr) {
    ...
    if (options.s3d_scan) {
      ...
      std::size_t fname_pos = file->file_name().rfind("/");
      std::string fname = homedir + string("/") + file->file_name().substr(fname_pos);
      ifstream ifs(fname.c_str(), std::ifstream::binary);

      ifs.seekg(0, ifs.end);
      size_t len = (int)ifs.tellg();
      uint8_t *s3d_buff = static_cast<uint8_t *>(malloc(len));
      
      ifs.seekg(0, ifs.beg);
      ifs.read(reinterpret_cast<char *>(s3d_buff), len);
      ifs.close();

      len = handle.size();
      uint8_t *s3d_page_buff = static_cast<uint8_t *>(malloc(len));
      memcpy(s3d_page_buff, s3d_buff + handle.offset(), len);
      contents = BlockContents(Slice(reinterpret_cast<const char *>(s3d_page_buff), len));
...
Status BlockBasedTable::MaybeReadBlockAndLoadToCache(
  ...
  if (block_cache != nullptr || block_cache_compressed != nullptr) {
    ...
    // file.
#ifdef VERSION_INDEX
    bool do_read = for_compaction || ro.fill_cache;
    if (block_entry->GetValue() == nullptr &&
        block_entry->GetCacheHandle() == nullptr && !no_io && do_read) {
#else
    if (block_entry->GetValue() == nullptr &&
...
#ifdef VERSION_INDEX
    bool no_insert = no_io || !(for_compaction || ro.fill_cache);
#else
...
Status BlockBasedTable::RetrieveBlock(
...
#if 0
  fprintf(stderr, "[SMARTSSD] RetrieveBlock at %s(%d)\n",
      __FILE__, __LINE__);
#endif
...

  {
    ...
    s = ReadBlockFromFile(
        rep_->file.get(), 
#ifdef SMARTSSD
  ro.s3d_scan ? nullptr : prefetch_buffer,
#else  
  prefetch_buffer, 
#endif
        rep_->footer, ro, handle, &block,
...
Status BlockBasedTable::Get(const ReadOptions& read_options, const Slice& key,
  ...
  if (!may_match) {
    ...
    for (iiter->Seek(key); iiter->Valid() && !done; iiter->Next()) {
      ...
      if (done) {
#ifdef VERSION_INDEX
        // Save address for write batch
        if (matched && vi_ctx != nullptr && vi_ctx->IsUserTable()) {
          const size_t n = biter.key().size();
          uint32_t xmax_offset = v.handle.offset()
            + biter.CurrentKeyOffset() + n - kNumInternalBytes
            + sizeof(SequenceNumber);

          vi_ctx->MarkSST(biter.key(), rep_->file->file_name(), xmax_offset);

#ifdef VERSION_INDEX_DEBUG_SYSBENCH
          // Validation code for checking the offset of the key-value in SST.
          int sst_fd = open(vi_ctx->fname.c_str(), O_RDONLY, (mode_t)0600);
          assert(sst_fd > 0);
          char buf[4];
          int ret = pread(sst_fd, buf, 4, vi_ctx->off - 12);
          close(sst_fd);
          assert(ret == 4);
          if (xmax_offset != vi_ctx->off) {
            std::cout << "Update key :" << Decode4ByteKey(buf) << ", "
              << Decode4ByteKey(biter.key().data() + 4) << std::endl;
          }
          assert(Decode4ByteKey(buf) ==
                 Decode4ByteKey(biter.key().data() + 4));
