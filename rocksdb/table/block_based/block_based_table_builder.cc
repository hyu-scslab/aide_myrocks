...
#ifdef VERSION_INDEX
  WritableFileWriter* vi_file;
  std::atomic<uint32_t> vi_offset;
  bool is_system = true;
#endif
...
#ifdef VERSION_INDEX
  uint64_t get_vi_offset() { return vi_offset.load(std::memory_order_relaxed); }
  void set_vi_offset(uint32_t o) { vi_offset.store(o, std::memory_order_relaxed); }
#endif
...
#ifdef VERSION_INDEX
  rep_->is_system = (tbo.column_family_id < 2);
  rep_->data_block.is_system = rep_->is_system;
#endif
...
#ifdef VERSION_INDEX
void BlockBasedTableBuilder::SetViFile(WritableFileWriter* file) {
  Rep* r = rep_;
  r->vi_file = file;
}
#endif
...
void BlockBasedTableBuilder::WriteBlock(BlockBuilder* block,
...
#ifdef VERSION_INDEX
  std::string vi_raw_block_contents;
  if (!rep_->is_system) {
    vi_raw_block_contents.reserve(block->EstimateSizeOfVersionIndex());
    block->ViSwapAndReset(vi_raw_block_contents);
  }
#endif
  ...
  if (!rep_->is_system) {
    if (block_type == BlockType::kData) {
      WriteViBlock(vi_raw_block_contents);
    }
  }
#endif
}
...
#ifdef VERSION_INDEX
void BlockBasedTableBuilder::WriteViBlock(const Slice& vi_block_contents) {
  Rep* r = rep_;

  assert(r != nullptr);
  assert(r->state == Rep::State::kUnbuffered);
  assert(r->vi_file != nullptr);
  assert(vi_block_contents.size() != 0);

  r->set_vi_offset(r->get_vi_offset() + vi_block_contents.size());

  IOStatus io_s = r->vi_file->Append(vi_block_contents);
  assert(io_s.ok());
}
#endif
