...
#ifdef VERSION_INDEX
  std::unique_ptr<WritableFileWriter> vi_outfile;
#endif
...
#ifdef VERSION_INDEX
  if (IsUserTable(c->column_family_data()->GetID())) {
    sst_dependency_mutex.Lock();
    for (auto &input : *c->inputs()) {
      for (auto &file : input.files) { // rocksdb::FileMetaData *
        uint64_t fnum = file->fd.GetNumber();
        auto it = sst_dependency.find(fnum);
        if (it != sst_dependency.end()) {
          // it->second->cnt++;
          continue;
        }
        // Initially, the is_done flag is set to false to indicate that
        // the compaction is in progress. Check class CompactionDependency.
        // We fill in the output file list in CompactionJob::Install phase.
        sst_dependency.insert(std::make_pair(fnum, new CompactionDependency()));
#ifdef VERSION_INDEX_DEBUG
        std::cout << "[ViPrescreen] Compaction (Start): " << fnum
                  << ".sst, writes blocked " << std::endl;
#endif
      }
    }
...
#ifdef VERSION_INDEX
  // REQUIRED: before CleanupCompaction() since cleanup deletes compaction object.
  if (IsUserTable(cfd->GetID())) {
    sst_dependency_mutex.Lock();
    std::unordered_set<uint64_t> output_fnums;
    for (const auto& state : compact_->sub_compact_states) {
      for (const auto& output : state.outputs) {
        uint64_t fnum = output.meta.fd.GetNumber();
        // TODO: Might fail if subcompaction is on.
        assert(output_fnums.find(fnum) == output_fnums.end());
        output_fnums.insert(fnum);
      }
    }

    bool cleanup = (output_fnums.size() == 0);

    for (auto &input : *compact_->compaction->inputs()) {
      for (auto &file : input.files) { // rocksdb::FileMetaData *
        uint64_t fnum = file->fd.GetNumber();
        auto iter = sst_dependency.find(fnum);
        assert(iter != sst_dependency.end());

        CompactionDependency *dep = iter->second;
        assert(dep != nullptr);
        ...
        if (cleanup) {
          ...
          __sync_synchronize();
          // TODO: delete logic implement, currently tracking memory leak
          // sst_dependency.erase(iter);
          // delete dep;
        } else {
          ...
          dep->outputs = output_fnums;
          assert(dep->outputs.size() > 0);
          __sync_synchronize();
          // dep->cnt--;
          dep->is_done.store(true, std::memory_order_relaxed);
#ifdef VERSION_INDEX_DEBUG
          std::cout << "[ViPrescreen] Compaction (End): " << fnum
                    << ", writes unblocked" << std::endl;
#endif
        }
      }
    }
...
#ifdef VERSION_INDEX
  std::unique_ptr<FSWritableFile> vi_writable_file;
#endif
#ifndef NDEBUG
...
#ifdef VERSION_INDEX
  FileOptions vi_fo_copy = file_options_;
  Status vi_s;

  IOStatus vi_io_s = NewWritableFile(fs_.get(), vi_fname, &vi_writable_file,
      vi_fo_copy);
  vi_s = vi_io_s;
...
#ifdef VERSION_INDEX
  vi_writable_file->SetIOPriority(GetRateLimiterPriority());
  vi_writable_file->SetWriteLifeTimeHint(write_hint_);
  sub_compact->vi_outfile.reset(new WritableFileWriter(
      std::move(vi_writable_file), vi_fname, vi_fo_copy, db_options_.clock,
      nullptr /* io_tracers */, db_options_.stats,
...
#ifdef VERSION_INDEX
  sub_compact->builder->SetViFile(sub_compact->vi_outfile.get());
#endif
  LogFlush(db_options_.info_log);
