...
#ifdef VERSION_INDEX
      char* xmax_address =
        const_cast<char*>(key_.data()) + key_.size() - kNumInternalBytes + 8;
      current_key_.UpdateXmax(xmax_address);
#endif
  ...
  if (valid_) {
    ...
    if (valid_ && compaction_ != nullptr &&
      ...
      if (!timestamp_size_) {
        ...
        // We skip zeroing for version index. We turn off compression in the first
        // place so there's no meaning of zeroing, and we also use the sequence
        // number in finding the desired key during write xmax.
