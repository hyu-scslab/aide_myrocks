...
uint64_t MemTableList::PickVisibleMaxMemTablesIDToFlush(
    SequenceNumber reader_seq) {
  const auto& memlist = current_->memlist_;

  // iterting from first imm to last imm
  // memlist's first is lastest memtable
  // Traversing, memtable from the latest to the earliest
  // when we meet the first visible memtable then return its ID
  for (auto it = memlist.begin(); it != memlist.end(); ++it) {
    MemTable* m = *it;

    if (m->GetEarliestSequenceNumber() <= reader_seq) {
      return m->GetID();
    }
  }
...
#ifdef SMARTSSD_FLUSH_MEMTABLE
// Returns the memtables that need to be flushed.
int MemTableList::GetNumberMemTablesToFlush(uint64_t max_memtable_id) {
  const auto& memlist = current_->memlist_;
  int num_mems = 0; 
