...
void DBIter::Next() {
  ...
  num_internal_keys_skipped_ = 0;
  bool ok = true;
  static uint64_t next_found_count = 1;
  ...
  if (!valid_) {
    if (statistics_) {
      // First record found by seek!
      fprintf(stderr, "Stat next found count : %ld\n", local_stats_.next_found_count_ + 1);
    } else {
      fprintf(stderr, "Next found count : %ld\n", next_found_count);
      next_found_count = 1;
    }
  }
#endif
...
#ifdef SMARTSSD_FLUSH_MEMTABLE
  bool ndp_iterating = read_options_.s3d_scan;
#endif /* SMARTSSD_FLUSH_MEMTABLE */
...
#ifdef SMARTSSD_FLUSH_MEMTABLE
    if (IsVisible(ikey_.sequence, ts, &more_recent) || ndp_iterating)
#else
      ...
      if ((!is_prev_key_seqnum_zero || timestamp_size_ > 0) &&
        ...
        if (!ndp_iterating) {
          assert(!skipping_saved_key ||
                 CompareKeyForSkip(ikey_.user_key, 
