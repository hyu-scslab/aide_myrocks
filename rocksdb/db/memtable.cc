...
#ifdef VERSION_INDEX
      is_user_table_(column_family_id > 1),
#endif
...
MemTable::~MemTable() {
  ...
  if (is_user_table_) {
    ...
    assert(live_memtables.find(id_) != live_memtables.end());
    live_memtables.erase(id_);
...
Slice MemTableRep::UserKey(const char* key) const {
  ...
  return Slice(slice.data(), slice.size() - kNumInternalBytes);
}
...
Status MemTable::VerifyEncodedEntry(Slice encoded,
  ...
  if (ikey_len < kNumInternalBytes + ts_sz) {
  ...
  const size_t user_key_len = ikey_len - kNumInternalBytes;
...
#ifdef VERSION_INDEX
  // Remove the next 12 byte for xmax and back pointer
  encoded.remove_prefix(kNumInternalBytes - 8);
#endif
...
Status MemTable::Add(SequenceNumber s, ValueType type,
...
#ifdef VERSION_INDEX
  // |key|xmin|type|xmax|backptr|
  // |<------internal key------>|
  internal_key_size += 8; // xmax (actually 7 byte for SequenceNumber)
  internal_key_size += 4; // back pointer (4 byte for offset)
#endif
  ...
  p += 8;
#ifdef VERSION_INDEX
  uint64_t xmax_packed = kMinUnCommittedSeq;
  EncodeFixed64(p, xmax_packed);
  p += 8;
  uint32_t offset = 0;
...
static bool SaveValue(void* arg, const char* entry) {
  ...
  assert(key_length >= kNumInternalBytes);
  Slice user_key_slice = Slice(key_ptr, key_length - kNumInternalBytes);
  ...
  if (user_comparator->EqualWithoutTimestamp(user_key_slice,
                                             s->key->user_key())) {
    // Correct user key
    const uint64_t tag = DecodeFixed64(key_ptr + key_length - kNumInternalBytes);
    ValueType type;
    SequenceNumber seq;
      ...
      case kTypeBlobIndex:
        ...
        if (!s->status->ok()) {
          *(s->found_final_value) = true;
          ...
          if (vi_ctx != nullptr && s->value != nullptr &&
              vi_ctx->IsUserTable()) {
            char* xmax_address =
              const_cast<char*>(key_ptr) + key_length - kNumInternalBytes
                                         + sizeof(SequenceNumber);
            vi_ctx->MarkMemTable(Slice(key_ptr, key_length), s->mem->GetID(),
                                 xmax_address);
          }
#endif
          return false;
...
Status MemTable::Update(SequenceNumber seq, const Slice& key,
  ...
  if (iter->Valid()) {
    ...
    if (comparator_.comparator.user_comparator()->Equal(
            Slice(key_ptr, key_length - kNumInternalBytes), lkey.user_key())) {
      // Correct user key
      const uint64_t tag = DecodeFixed64(key_ptr + key_length - kNumInternalBytes);
      ValueType type;
...
size_t MemTable::CountSuccessiveMergeEntries(const LookupKey& key) {
  ...
  for (; iter->Valid(); iter->Next()) {
    ...
    if (!comparator_.comparator.user_comparator()->Equal(
            Slice(iter_key_ptr, key_length - kNumInternalBytes), key.user_key())) {
      break;
    }

    const uint64_t tag = DecodeFixed64(iter_key_ptr + key_length - kNumInternalBytes);
    ValueType type;
    uint64_t unused;
