...
#ifdef SMARTSSD
int ColumnFamilyHandleImpl::GetTableId() {
  int rel_id;
  sscanf(cfd()->GetName().c_str(), "cf_name=cf%d", &rel_id);
  return rel_id;
}

int ColumnFamilyHandleImpl::CountFiles() {
  if (file_count_ < 0) {
    file_count_ = cfd()->current()->CountSstFiles();
  }
  return file_count_;
}

void ColumnFamilyHandleImpl::GetFiles(std::vector<std::string> *files) {
  cfd()->current()->GetFiles(files);
}
#endif //SMARTSSD

#ifdef VERSION_INDEX
void ColumnFamilyHandleImpl::SetNullBytes(int nullbytes) {
  cfd()->nullbytes = nullbytes;
}
#endif
