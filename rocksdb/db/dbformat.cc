...
#ifdef VERSION_INDEX
  PutFixed64(result, kMinUnCommittedSeq);
  PutFixed32(result, 0);
#endif
}
...
#ifdef VERSION_INDEX
    PutFixed64(&tmp, kMinUnCommittedSeq);
    PutFixed32(&tmp, 0);
#endif
...
LookupKey::LookupKey(const Slice& _user_key, SequenceNumber s,
  ...
  dst = EncodeVarint32(dst, static_cast<uint32_t>(usize + ts_sz + kNumInternalBytes));
  kstart_ = dst;
  ...
  dst += 8;
#ifdef VERSION_INDEX
  EncodeFixed64(dst, kMinUnCommittedSeq);
  dst += 8;
  EncodeFixed32(dst, 0);
  dst += 4;
#endif
