...
#ifdef VERSION_INDEX
// kNumInternalBytes + xmax (8) + back ptr (4)
constexpr uint32_t kNumInternalBytes = 20;

inline bool IsSystemTableKey(const Slice &slice) {
  return slice.size() <= kNumInternalBytes;
}
#else
...
#ifdef VERSION_INDEX
inline SequenceNumber ExtractSeq(const Slice& internal_key) {
  uint64_t num = ExtractInternalKeyFooter(internal_key);
  return num;
}

inline SequenceNumber ExtractXmin(const Slice& internal_key) {
  return ExtractSeq(internal_key) >> 8;
}

inline SequenceNumber ExtractXmax(const Slice& internal_key) {
  assert(internal_key.size() >= kNumInternalBytes);
  const size_t n = internal_key.size();
  return DecodeFixed64(internal_key.data() + n - kNumInternalBytes + 8);
}

inline uint32_t ExtractOffset(const Slice& internal_key) {
  assert(internal_key.size() >= kNumInternalBytes);
  const size_t n = internal_key.size();
  return DecodeFixed32(internal_key.data() + n - kNumInternalBytes + 16);
}
#endif
...
class IterKey {
 public:
...
#ifdef VERSION_INDEX
  const char* GetKeyAddress() const { return key_; }
#endif
...
#ifdef VERSION_INDEX
  void UpdateXmax(char* xmax) {
    assert(key_size_ > kNumInternalBytes);
    memcpy(&buf_[key_size_ - kNumInternalBytes + 8], xmax, sizeof(SequenceNumber));
  }
#endif
  ...
  void SetInternalKey(const Slice& key_prefix, const Slice& user_key,
    ...
    key_ = buf_;
    key_size_ = psize + usize + kNumInternalBytes + ts_sz;
    is_user_key_ = false;
  }
