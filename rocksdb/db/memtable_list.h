...
class MemTableList {
 public:
  ...
  uint64_t PickVisibleMaxMemTablesIDToFlush(SequenceNumber reader_seq);
  int GetNumberMemTablesToFlush(uint64_t max_memtable_id);
  ...
  bool AssignAtomicFlushSeqForVisibleMemTable(const SequenceNumber& seq,
    ...
    for (auto it = memlist.rbegin(); it != memlist.rend(); ++it) {
      MemTable* mem = *it;
      if (mem->GetEarliestSequenceNumber() <= snapshot) {
        if (mem->atomic_flush_seqno_ == kMaxSequenceNumber) {
          mem->atomic_flush_seqno_ = seq;
          need_flush = true;
        }
