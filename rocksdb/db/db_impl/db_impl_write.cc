...
// Get the column family lists with visible memtable and
// assign sequence number for atomic flush.
void DBImpl::AssignAtomicFlushSeqForVisibleMemTable(
  ...
  for (auto cfd : cfds) {
    if (cfd->imm()->AssignAtomicFlushSeqForVisibleMemTable(seq, 
                                                           seq_nos[idx])) {
      cfd->imm()->FlushRequested();
      flush_cfds.emplace_back(cfd);
      flush_seq_nos.emplace_back(seq_nos[idx]);
    }
    idx++;
  }
}
    ...
    for (auto cfd : cfds) {
      ...
      if (!immutable_db_options_.atomic_flush) {
        ...
        for (uint64_t j = cfd->imm()->GetEarliestMemTableID() ;
             j <= cfd->imm()->GetLatestMemTableID(); j++) {
          FlushRequest flush_req {{cfd, j}};
          flush_reqs.emplace_back(std::move(flush_req));
        }
        for (const auto& req: flush_reqs) {
          SchedulePendingFlush(req, FlushReason::kWalFull);
        }
#else
