...
uint64_t DBImpl::MinObsoleteSstNumberToKeep() {
  ...
  uint64_t aide_min = std::numeric_limits<uint64_t>::max();
  uint64_t db_min = std::numeric_limits<uint64_t>::max();

  if (!pending_outputs_.empty()) {
    db_min = *pending_outputs_.begin();
  }

  if (!min_aide_pending_outputs_.empty()) {
    aide_min = *min_aide_pending_outputs_.begin();
  }

  return (db_min > aide_min) ? aide_min : db_min;
#else
  if (!pending_outputs_.empty()) {
...
#ifdef SMARTSSD_FLUSH_MEMTABLE
  std::list<uint64_t>& aide_pending_outputs = get_pending_output();
