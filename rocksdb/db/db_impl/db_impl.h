...
class DBImpl : public DB {
 public:
  ...
  /*
   * entry point for AIDE functions in rocksdb from mysql
   */
  public:
    /* flush requesy for aide */
  Status FlushRequestForAIDE(
      ColumnFamilyHandle* column_family,
  ...
  Status FlushRequestForAIDE(
      ...
      std::vector<std::vector<std::string>*>& fname_vector);
    /* flush requesy for aide */
  Status WaitForFlushVisibleMemTables(
  ...
  Status WaitForFlushVisibleMemTables(
      ...
      uint64_t earliest_memids,
      uint64_t latest_memids,
      std::vector<uint64_t>* stall_file_deletes);
  void ReleaseAideInfo(
...
#ifdef SMARTSSD_FLUSH_MEMTABLE
  void ReleaseFileNumberFromPendingOutputs(uint64_t delete_stalled_num);
#endif
...
#ifdef SMARTSSD_FLUSH_MEMTABLE
private:
  using FileMetaList = std::map<uint64_t, FileMetaData* >;
public:
  /* Functions for visisble mem table flush.
   * These functions do 
   * 1) check if there are visible memtables
   * 2) make flush requests for visible memtables
   * 3) request flush
   * 4) wait for flush end
   */
  void GenerateFlushRequestForVisibleMemTable(
      const autovector<ColumnFamilyData*>& cfds,
      const autovector<SequenceNumber>& seqnos);
      //FlushRequest* req);
  Status FlushRequestVisibleMemTable(
      ColumnFamilyData* cfd,
      ...
      std::vector<uint64_t>* stall_file_deletes,
			std::vector<std::vector<std::string>*>& fname_vector,
      bool writes_stopped = true);
  Status FlushRequestVisibleMemTables(
        const autovector<ColumnFamilyData*> cfds,
        const autovector<SequenceNumber> seqnos,
        const FlushOptions& flush_options, 
        std::vector<uint64_t>& earliest_mem_ids,
        ...
        std::vector<uint64_t>* stall_file_deletes,
				std::vector<std::vector<std::string>*>& fname_vector);
  Status AtomicFlushRequestVisibleMemTables(
      const autovector<ColumnFamilyData*>& column_family_data,
      const autovector<SequenceNumber>& seq_no_data,
      const FlushOptions& flush_options, FlushReason& flush_reason,
      std::vector<int>& num_flush, std::vector<void*>& wait_cfd,
      std::vector<uint64_t>& wait_memid, bool writes_stopped = true);
  // REQUIRES: mutex locked and in write thread.
  // If atomic flush is being done, assign flush sequence number for memtable
  void AssignAtomicFlushSeqForVisibleMemTable(
      const autovector<ColumnFamilyData*>& cfds,
      const autovector<SequenceNumber>& seq_nos,
  ...
  void GetAddedFiles(
      void* wait_cfds, std::vector<std::string>* fname_vectors,
      uint64_t earliest_ids, uint64_t latest_ids,
      std::vector<uint64_t>* stall_file_deletes);
      /* Acquire file name and its structures for aide */
  void ThrowAwayUnusedFileSnapshot(
      const autovector<ColumnFamilyData*>& cfds,
...
#ifdef SMARTSSD_FLUSH_MEMTABLE
  std::list<uint64_t> aide_pending_outputs_;
  std::list<uint64_t> min_aide_pending_outputs_;
  std::list<uint64_t>& get_pending_output() {
    return aide_pending_outputs_;
  }
#endif
