...
Status DBImpl::FlushMemTableToOutputFile(
    ...
    Env::Priority thread_pri, uint64_t requested_memtable_id) {
#else
Status DBImpl::FlushMemTableToOutputFile(
    ColumnFamilyData* cfd, const MutableCFOptions& mutable_cf_options,
    bool* made_progress, JobContext* job_context,
...
#ifdef SMARTSSD_FLUSH_MEMTABLE
  uint64_t max_memtable_id = requested_memtable_id;
#else
...
#ifdef VERSION_INDEX
    for (MemTable* m: flush_job.GetMemTables()) {
      // Create dependency
      if (m->IsUserTable()) {
        // TODO: just move the lock outside the for loop, it seems short enough.
        memtable_dependency_mutex.Lock();
        uint64_t mem_id = m->GetID();
        auto it = memtable_dependency.find(mem_id);
        if (it == memtable_dependency.end()) {
          memtable_dependency.insert(
              std::make_pair(mem_id, new CompactionDependency()));
        }
#ifdef VERSION_INDEX_DEBUG
        std::cout << "[ViPrescreen] Flush (Start): " << mem_id
                  << ", writes blocked" << std::endl;
#endif
        memtable_dependency_mutex.Unlock();
      }
    }
#endif
    need_cancel = true;
  ...
  if (s.ok()) {
    ...
    memtable_dependency_mutex.Lock();
    for (MemTable* m: flush_job.GetMemTables()) {
      if (!m->IsUserTable()) continue;

      uint64_t mem_id = m->GetID();
      auto it = memtable_dependency.find(mem_id);
      assert(it != memtable_dependency.end());

      CompactionDependency *dep = it->second;
      assert(dep != nullptr);
      assert(dep->is_done.load(std::memory_order_relaxed) == false);

      // The order of setting the output SST file number and the is_done flag
      // must be preserved. We look at the is_done flag in write_batch.cc to
      // ensure that the compaction is done. If we mix-up these two orders,
      // there will be a chance that the write_batch might miss the output list.
      dep->outputs.insert(file_meta.fd.GetNumber());
      __sync_synchronize();
      dep->is_done.store(true, std::memory_order_relaxed);
#ifdef VERSION_INDEX_DEBUG
      std::cout << "[ViPrescreen] Flush (End): " << mem_id << " -> "
                << file_meta.fd.GetNumber() << ".sst, writes unblocked " << std::endl;
#endif
    }
    memtable_dependency_mutex.Unlock();
#endif
...
#ifdef SMARTSSD_FLUSH_MEMTABLE
/*
 * Generating flush reqeusts for visible memtables
 */ 
void DBImpl::GenerateFlushRequestForVisibleMemTable(
      const autovector<ColumnFamilyData*>& cfds,
      const autovector<SequenceNumber>& seqnos) {
  // assert(req != nullptr);
  int i = 0;
  uint64_t flush_memtable_id;
  for (const auto cfd : cfds) {
    if (nullptr == cfd) {
      i++;
      continue;
    }
    // get the latest visible memtable id
    flush_memtable_id = cfd->imm()->PickVisibleMaxMemTablesIDToFlush(seqnos[i]);
    // set the id and cfd to flush requests
    // req->emplace_back(cfd, flush_memtable_id);
    i++;

    for (uint64_t j = cfd->imm()->GetEarliestMemTableID() ;
         j <= flush_memtable_id ; j++) {
      FlushRequest flush_req{{cfd, j}};
      // requests flush
      SchedulePendingFlush(flush_req, FlushReason::kAIDE);
      MaybeScheduleFlushOrCompaction();
    }
  }
}

Status DBImpl::WaitForFlushVisibleMemTables(void* wait_cfd, uint64_t wait_memid,
        std::vector<std::string>* fname_vector, uint64_t earliest_memids,
        uint64_t latest_memids, std::vector<uint64_t>* stall_file_deletes) {
  Status s;

  autovector<ColumnFamilyData*> cfds;
  autovector<const uint64_t*> flush_memtable_ids;

  cfds.emplace_back((ColumnFamilyData*)wait_cfd);
  flush_memtable_ids.emplace_back(&wait_memid);

  //  Function call for wating !
  s = WaitForFlushMemTables(cfds, flush_memtable_ids, false);

  {
    InstrumentedMutexLock lock_guard(&mutex_);

    GetAddedFiles(wait_cfd, fname_vector, earliest_memids, latest_memids, 
        stall_file_deletes);
...
// Wait for completing flushing visible memtables
Status DBImpl::WaitForFlushVisibleMemTables(
        const std::vector<void*>& wait_cfd,
        std::vector<uint64_t>& wait_memid,
        std::vector<std::vector<std::string>*>& fname_vector,
        std::vector<uint64_t>& earliest_memids,
        std::vector<uint64_t>& latest_memids) {
  Status s;

  autovector<ColumnFamilyData*> cfds;
  autovector<const uint64_t*> flush_memtable_ids;

  assert(wait_cfd.size() == wait_memid.size());

  for (size_t i = 0 ; i < wait_cfd.size() ; i++) {
    cfds.emplace_back((ColumnFamilyData*)wait_cfd[i]);
    flush_memtable_ids.emplace_back(&(wait_memid[i]));
  }
  ...
  wait_memid.clear();
  earliest_memids.clear();
  latest_memids.clear();

  return s;
}

// If we want to flush many column familes's memtable tiedly up
// then, use this function
Status DBImpl::AtomicFlushRequestVisibleMemTables(
    const autovector<ColumnFamilyData*>& column_family_data,
    const autovector<SequenceNumber>& seq_no_data,
  ...
  // In PoC, never exeute this code segment
  if (!flush_options.allow_write_stall) {
    int num_cfs_to_flush = 0;
    for (auto cfd : column_family_data) {
  ...
  autovector<ColumnFamilyData*> cfds;
  autovector<SequenceNumber> seq_nos;
  {
    ...
    // This thread shoule be currently at the front of the writer queue
    ...
    for (auto cfd : column_family_data) {
      ...
      // If there's in-memory cached data, then add flush cfd list
      if (cfd->imm()->NumNotFlushed() != 0 || !cfd->mem()->IsEmpty() ||
          !cached_recoverable_state_empty_.load()) {
        cfds.emplace_back(cfd);
        seq_nos.emplace_back(seq_no_data[idx++]);
      }
    }
    idx = 0;
    autovector<ColumnFamilyData*> flush_cfds;
    autovector<SequenceNumber> flush_seq_nos;

    /* check the memtable of cf has visible data, then set it to immutable */
    for (auto cfd : cfds)  {
      if (((cfd->mem()->IsEmpty() && cached_recoverable_state_empty_.load()) ||
          flush_reason == FlushReason::kErrorRecoveryRetryFlush) &&
          (cfd->mem()->GetEarliestSequenceNumber() > seq_nos[idx])) {
        idx++;
        continue;
      }

      cfd->Ref();
      // change visible memtable to immutable memtable
    ...
    if (s.ok()) {
      // get the column families that have visible memtables
      // and assign flush sequence number
      AssignAtomicFlushSeqForVisibleMemTable(cfds, seq_nos, flush_cfds, 
                                             flush_seq_nos);

      if (flush_options.wait) {
        for (auto cfd : flush_cfds) {
          cfd->Ref();
        }
      }
      // generate flush request tying up column famalies
      GenerateFlushRequestForVisibleMemTable(flush_cfds, flush_seq_nos);
      //                                       &flush_req);
    }
  ...
  if (s.ok() && flush_options.wait) {
    ...
    for (auto& iter : flush_req) {
      ...
      wait_cfd.push_back(iter.first); 
      wait_memid.push_back(iter.second);
      num_flush.push_back(
          iter.first->imm()->GetNumberMemTablesToFlush(iter.second));
    }
  }
  return s;
}

// Flush memtables per column family
// cfd - column family data for flushing
// seqno - reader's sequence number. if memtable's earliest sequence number is
//         lower than this, then that memtable will be flushed
// flush_options - options for flushing. For AIDE flush should be executed 
//                 immediately. so allow_write_stall is turn no.
// flush_reason - for notifying storage engines and other bg workers of 
//                triggering flush for AIDE
// writes_stopped - In PoC, always true. do not wait for write batching
Status DBImpl::FlushRequestVisibleMemTable(
          ...
          std::vector<void*>& wait_cfd,
          std::vector<uint64_t>& wait_memid,
          autovector<Version*>& file_snapshots,
          std::vector<uint64_t>* stall_file_deletes,
          std::vector<std::vector<std::string>*>& fname_vector,
          bool writes_stopped) {
  assert(!immutable_db_options_.atomic_flush);
  Status s;

  // Always allow_write_stall is true, because AIDE needs memtable data 
  // immediately
  if (!flush_options.allow_write_stall) {
    // if this function call doesn't allows to stall writing, then follow this 
    // logic.
    bool flush_needed = true;
    // Wait until this flush requests will not stall write
    s = WaitUntilFlushWouldNotStallWrites(cfd, &flush_needed);
  ...
  autovector<FlushRequest> flush_reqs; /* flush requests */
  autovector<uint64_t> memtable_ids_to_wait; /* wait flush memtable id */

  {
    WriteContext context;
    InstrumentedMutexLock guard_lock(&mutex_); /* acquire mutex for flushing */
    ...
    Version* snaps = cfd->current();

    snaps->PrintRef();
  
    const ImmutableOptions& ioptions = *cfd->ioptions();
    snaps->GetAllSstFileName(ioptions, fname_vector);
    file_snapshots.emplace_back(snaps);

    for (auto fname : *fname_vector[fname_vector.size() - 1]) {
      uint64_t ids;
      sscanf(fname.c_str(), "./.rocksdb/%lu.sst", &ids);
      if (stall_file_deletes) {
        if (find(stall_file_deletes->begin(), stall_file_deletes->end(), ids)
            == stall_file_deletes->end()) {
          aide_pending_outputs_.push_back(ids);
          stall_file_deletes->push_back(ids);
          fprintf(stderr, "blocking output elem : %lu\n", ids);
        }
      }
    }

    uint64_t earliest_flush_memtable_id = cfd->imm()->GetEarliestMemTableID();
    uint64_t flush_memtable_id;
    // if current memtable is not empty and visible, then change it to immutable
    if (flush_reason != FlushReason::kErrorRecoveryRetryFlush &&
        !cfd->mem()->IsEmpty() &&
        cfd->mem()->GetEarliestSequenceNumber() <= seqno) {

      // For switching memtable to immutable, if memtable is not
      s = SwitchMemtable(cfd, &context);
      if (s.ok()) {
        // Get the current latest memtable id
        // In RocksDB, immutable memtables are chained as below:
        // Latest_imm-> ... -> 3rd_imm -> 2nd_imm -> earliest_imm
        // Memtable id is increase atomically
        // And the latest immutable memtable was switched just before 
        // We wait until flushing lastest memtable so set its ID.
        flush_memtable_id =
            cfd->imm()->PickVisibleMaxMemTablesIDToFlush(seqno);
        earliest_flush_memtable_id = cfd->imm()->GetEarliestMemTableID();
        if (std::numeric_limits<uint64_t>::max() ==
            earliest_flush_memtable_id) {
          assert(0);
        }
        ...
        if (stall_file_deletes) {
          ...
          fprintf(stderr, "pushing pending output elem : %lu\n",
              versions_->current_next_file_number());
          min_aide_pending_outputs_.push_back(current_next_file_number);
        }
      } else {
        assert(0);
      }
    } else if (cfd->imm()) { /* TODO: processing cfd->imm() is null */
      // Pick visible id
      // Latest_imm-> ... -> 3rd_imm -> 2nd_imm -> earliest_imm
      //                  ^ - visible 
      // if 3rd immutable memtable is visible, memtable 1,2,3 is visible for 
      // readers. so, return 3rd_imm's memtable id
      flush_memtable_id =
          cfd->imm()->PickVisibleMaxMemTablesIDToFlush(seqno);
      if (flush_memtable_id == std::numeric_limits<uint64_t>::max()) {
        num_flush.push_back(0);
        return s;
      }
      earliest_flush_memtable_id = cfd->imm()->GetEarliestMemTableID();
      assert(std::numeric_limits<uint64_t>::max() != 
             earliest_flush_memtable_id);
      // For waiting until flushing visbile memtable, set its id
      memtable_ids_to_wait.emplace_back(flush_memtable_id);
    ...
    if (s.ok()) {
      num_flush.push_back(
          cfd->imm()->GetNumberMemTablesToFlush(flush_memtable_id));
      if (cfd->imm()->NumNotFlushed() != 0 || !cfd->mem()->IsEmpty() ||
          !cached_recoverable_state_empty_.load()) {
        //check if there's a not flushed yet
        earliest_mem_ids.push_back(earliest_flush_memtable_id);
        latest_mem_ids.push_back(flush_memtable_id);
        for (uint64_t i = earliest_flush_memtable_id ; i <= flush_memtable_id ; 
             i++) {
          FlushRequest req{{cfd, i}}; 
          flush_reqs.emplace_back(std::move(req));
        }
      }
      // If We need to flush stats table
      if (immutable_db_options_.persist_stats_to_disk &&
        ...
        if (cfd_stats != nullptr && cfd_stats != cfd &&
          ...
          // only force flush stats CF when it will be the only CF logging
          ...
          if (stats_cf_flush_needed) {
            // For flushing stat memtable, chagne stat memtable to immutable
            ROCKS_LOG_INFO(immutable_db_options_.info_log,
                           "Force flushing visible memtable stats CF with "
                           "flush of %s to avoid holding old logs",
                           cfd->GetName().c_str());
    ...
    // Finally, make flush request and request it
    if (s.ok() && !flush_reqs.empty()) {
      // If we need to flush memtable
      ...
      // Always flush_options.wait is true
      if (flush_options.wait) {
        for (const auto& req: flush_reqs) {
          assert(req.size() == 1);
          ColumnFamilyData* loop_cfd = req[0].first;
  ...
  if (s.ok() && flush_options.wait) {
    wait_cfd.push_back(flush_reqs[flush_reqs.size() - 1][0].first);
    wait_memid.push_back(memtable_ids_to_wait[flush_reqs.size() - 1]);
  }
  return s;
}

Status DBImpl::FlushRequestVisibleMemTables(
        const autovector<ColumnFamilyData*> cfds,
        const autovector<SequenceNumber> seqnos,
        const FlushOptions& flush_options, 
        std::vector<uint64_t>& earliest_mem_ids,
        ...
        std::vector<void*>& wait_cfd,
        std::vector<uint64_t>& wait_memid,
        autovector<Version*>& file_snapshot,
        std::vector<uint64_t>* stall_file_deletes,
        std::vector<std::vector<std::string>*>& fname_vector) {
  Status s;
  ...
  int i = 0;
  for (auto cfd: cfds) {
    SequenceNumber seqno = seqnos[i++];
    if (cfd->IsDropped()) {
      continue;
    }
    // flush visible memtables per column family
    s = FlushRequestVisibleMemTable(cfd, seqno, flush_options, flush_reason,
            earliest_mem_ids, latest_mem_ids, num_flush, wait_cfd, wait_memid,
            file_snapshot, stall_file_deletes, fname_vector);
    if (!s.ok()) {
      break;
    }
  }
...
Status DBImpl::FlushRequestForAIDE(
    ...
    std::vector<void*>& wait_cfd,
    std::vector<uint64_t>& wait_memid,
    std::vector<void*>& wait_file_snapshots,
    std::vector<uint64_t>& pending_output_files,
    std::vector<std::vector<std::string>*>& fname_vector) {
    std::vector<ColumnFamilyHandle*> column_families;
    std::vector<SequenceNumber> visible_sequence_numbers;

    column_families.push_back(column_family);
    visible_sequence_numbers.push_back(seq_no);

  return FlushRequestForAIDE(column_families, visible_sequence_numbers,
            earliest_mem_ids, latest_mem_ids, num_flush, wait_cfd, 
            wait_memid, wait_file_snapshots, pending_output_files,
            fname_vector);
}

/*
 * entry point of visble memtable flush function for AIDE
 */
Status DBImpl::FlushRequestForAIDE(
        const std::vector<ColumnFamilyHandle*>& column_families,
        const std::vector<SequenceNumber>& visible_sequence_numbers,
        std::vector<uint64_t>& earliest_mem_ids,
        std::vector<uint64_t>& latest_mem_ids,
        std::vector<int>& num_flush,
        std::vector<void*>& wait_cfd,
  ...
  autovector<ColumnFamilyData*> cfds;
  autovector<SequenceNumber> seqnos;
  autovector<Version*> file_snapshot;

  reason = FlushReason::kAIDE;
  ...
  for (auto seqno : visible_sequence_numbers) {
    seqnos.emplace_back(seqno);
  }

  s = FlushRequestVisibleMemTables(cfds, seqnos, flush_opts,
        earliest_mem_ids, latest_mem_ids, reason, num_flush, wait_cfd, 
        wait_memid, file_snapshot, &pending_output_files, fname_vector);

  // extract file snapshot that don't need to wait
  ThrowAwayUnusedFileSnapshot(cfds, file_snapshot, wait_cfd, 
      wait_file_snapshots, fname_vector);
...
#ifdef SMARTSSD_FLUSH_MEMTABLE
  uint64_t requested_memtable_id = bg_flush_arg.max_memtable_id_;
  Status s = FlushMemTableToOutputFile(
      cfd, mutable_cf_options_copy, made_progress, job_context,
      superversion_context, snapshot_seqs, earliest_write_conflict_snapshot,
      snapshot_checker, log_buffer, thread_pri, requested_memtable_id);
#else
  Status s = FlushMemTableToOutputFile(
      cfd, mutable_cf_options_copy, made_progress, job_context,
      superversion_context, snapshot_seqs, earliest_write_conflict_snapshot,
  ...
  autovector<uint64_t> memtable_ids_to_wait;
  {
...
#ifdef SMARTSSD_FLUSH_MEMTABLE
    const uint64_t flush_memtable_id = cfd->imm()->GetLatestMemTableID();
#else
    ...
    if (s.ok()) {
      if (cfd->imm()->NumNotFlushed() != 0 || !cfd->mem()->IsEmpty() ||
          !cached_recoverable_state_empty_.load()) {
#ifdef SMARTSSD_FLUSH_MEMTABLE
        uint64_t earliest_flush_memtable_id = 
