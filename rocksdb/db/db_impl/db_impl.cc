...
#ifdef SMARTSSD
#ifdef WITH_NDP_LIB
  if (read_options.s3d_scan) {
    int id;
    sscanf(cfd->GetName().c_str(), "cf_name=cf%u", &id);
    merge_iter_builder.AddIterator(new NDPIterator(cfd->es_ndpo_, id, 
        read_options, read_options.ndp_key_size));
...
void DBImpl::ReleaseFileNumberFromPendingOutputs(uint64_t delete_stalled_num) {
  auto it_apo = std::find(aide_pending_outputs_.begin(),
                aide_pending_outputs_.end(), delete_stalled_num);

  if (it_apo != aide_pending_outputs_.end()) {
    fprintf(stderr, "releasing output elem : %lu\n", *it_apo);
    aide_pending_outputs_.erase(it_apo);
  } else {
    assert(0);
  }
}
...
#ifdef SMARTSSD_FLUSH_MEMTABLE
void DBImpl::ThrowAwayUnusedFileSnapshot(
  const autovector<ColumnFamilyData*>& cfds,
  const autovector<Version*>& file_snapshots,
  std::vector<void*>& wait_cfd,
  std::vector<void*>& wait_file_snapshots,
  std::vector<std::vector<std::string>*>& fname_vector) {
  
  for (auto file_snapshot: file_snapshots) {
    wait_file_snapshots.push_back(file_snapshot);
  }
}

// Release resources for aide
void DBImpl::ReleaseAideInfo(
    std::vector<void*>& wait_file_snapshots,
    std::vector<std::vector<std::string>*>& fname_vector,
    std::vector<uint64_t>* stall_file_deletes) {
  {
    ...
    for (size_t i = 0 ; i < wait_file_snapshots.size() ; i++) {
      Version * ref_v = (Version*)wait_file_snapshots[i];
      ref_v->Unref();
    }

    wait_file_snapshots.resize(0);

    if (stall_file_deletes) {
      for (size_t i = 0 ; i < stall_file_deletes->size() ; i++) {
        uint64_t stalled_file_number = (*stall_file_deletes)[i];
        ReleaseFileNumberFromPendingOutputs(stalled_file_number);
      }
    }
  }
}

void DBImpl::GetAddedFiles(
  void* wait_cfds, std::vector<std::string>* fname_vector,
  uint64_t earliest_id, uint64_t latest_id,
  std::vector<uint64_t>* stall_file_deletes) {

  for (uint64_t i = earliest_id ; i <= latest_id ; i++) {
#if 0
      auto fm = fm_list->find(i)->second;
      fname_vector->push_back(TableFileName(cfd->ioptions()->cf_paths,
          fm->fd.GetNumber(), fm->fd.GetPathId()));
#endif
#ifdef VERSION_INDEX
    memtable_dependency_mutex.Lock();
    auto dep_iter = memtable_dependency.find(i);
    assert(dep_iter != memtable_dependency.end());

    CompactionDependency* dep = dep_iter->second;
    assert(dep != nullptr);
    assert(dep->is_done.load(std::memory_order_relaxed) == true);

    // The current version outputs one immutable table to a single file.
    assert(dep->outputs.size() == 1);
    for (auto& output: dep->outputs) {
      std::string sst_name = MakeTableFileName("./.rocksdb", output);
      fname_vector->push_back(sst_name);
      if (stall_file_deletes) {
        if (find(stall_file_deletes->begin(), stall_file_deletes->end(), output)
            == stall_file_deletes->end()) {
          aide_pending_outputs_.push_back(output);
          stall_file_deletes->push_back(output);
  ...
  for (auto fname : *fname_vector) {
    fprintf(stderr,"%s\n", fname.c_str());
  }
  if (!min_aide_pending_outputs_.empty()) {
    fprintf(stderr, "extracting pending output elem : %lu\n",
            *min_aide_pending_outputs_.begin());
    min_aide_pending_outputs_.pop_front();
  }
}

void DBImpl::GetAddedFiles(
  const std::vector<void*>& wait_cfds,
  std::vector<std::vector<std::string>*>& fname_vectors,
  const std::vector<uint64_t>& earliest_ids,
  const std::vector<uint64_t>& latest_ids) {
  size_t i, j; 
  bool last = false;
  
  if (!earliest_ids.size()) {
    return;
  }

  assert(earliest_ids.size() == latest_ids.size());
  ColumnFamilyData* prev = nullptr;
  for (i = 0, j = 0 ; i < earliest_ids.size() ; i++, j++) {
    ColumnFamilyData* cfd = (ColumnFamilyData*)(wait_cfds[j]);
    while(prev == cfd) {
      j++;
      if (j >= wait_cfds.size()) {
        last = true;
      ...
      cfd = (ColumnFamilyData*)(wait_cfds[j]);
    }
    ...
    uint64_t earliest_id = earliest_ids[i];
    uint64_t latest_id = latest_ids[i];
    std::vector<std::string>* fname_vector = fname_vectors[i];
