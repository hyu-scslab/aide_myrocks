...
class ColumnFamilyHandleImpl : public ColumnFamilyHandle {
 public:
  ...
  virtual int CountFiles() override;
  virtual void GetFiles(std::vector<std::string> *files) override;
#endif //SMARTSSD
#ifdef VERSION_INDEX
  virtual void SetNullBytes(int nullbytes) override;
#endif
...
class ColumnFamilyData {
 public:
  ...
  void SetMemtable(MemTable* new_mem) {
    ...
    assert(live_memtables.find(memtable_id) == live_memtables.end());
    live_memtables.insert(memtable_id);
#endif
...
#ifdef VERSION_INDEX
  // For dependency tracking, we use unique memtable id for 
  // the entire column families.
  static inline std::atomic<uint64_t> last_memtable_id_{0};
#else
