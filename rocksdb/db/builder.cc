...
Status BuildTable(
  ...
  bool is_system = (tboptions.column_family_id < 2);
#endif
...
#ifdef VERSION_INDEX
    std::unique_ptr<WritableFileWriter> vi_file_writer;
#endif
    {
      ...
      assert(s.ok());
#ifdef VERSION_INDEX
      std::unique_ptr<FSWritableFile> vi_file;

      if (!is_system) {
        IOStatus vi_io_s = NewWritableFile(fs, vi_fname, &vi_file, file_options);
        assert(vi_io_s.ok());
      }
#endif
      s = io_s;
...
#ifdef VERSION_INDEX
      if (!is_system) {
        FileTypeSet vi_tmp_set = ioptions.checksum_handoff_file_types;
        vi_file->SetIOPriority(io_priority);
        vi_file->SetWriteLifeTimeHint(write_hint);
        vi_file_writer.reset(new WritableFileWriter(
            std::move(vi_file), vi_fname, file_options, ioptions.clock,
            nullptr /* io_tracers */, ioptions.stats,
            std::vector<std::shared_ptr<EventListener>>() /* ioptions.listener */,
            ioptions.file_checksum_gen_factory.get(),
            tmp_set.Contains(FileType::kTableFile), false));

        builder->SetViFile(vi_file_writer.get());
        builder->SetNullBytes(nullbytes);
      }
#endif
    }
...
#ifdef VERSION_INDEX_DEBUG_SYSBENCH
      if (tboptions.column_family_id > 1) {
        if (prev_key.size() > 0) {
          // Check if user key is same
          if (memcmp(prev_key.data(), key.data(), key.size() - kNumInternalBytes) == 0) {
            uint32_t user_key = Decode4ByteKey(&prev_key.data()[4]);
            std::cout << "[ViPrescreen] prev key (" << user_key << ") "
                      << ExtractXmin(key) << "~" << ExtractXmax(key) << ", "
                      << ExtractXmin(prev_key) << "~" << ExtractXmax(prev_key) << std::endl;
          }
        }
        memcpy(const_cast<char*>(prev_key.data()), key.data(), key.size());
      }
#endif
...
#ifdef VERSION_INDEX
    if (!is_system) {
      IOStatus vi_io_s = vi_file_writer->Sync(ioptions.use_fsync);
      assert(vi_io_s.ok());

      vi_io_s = vi_file_writer->Close();
      assert(vi_io_s.ok());
    }
#endif
    if (s.ok() && io_status->ok() && !empty) {
