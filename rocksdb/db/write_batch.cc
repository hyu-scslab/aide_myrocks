...
#ifdef VERSION_INDEX_DEBUG_XMAX_WRITER
      std::cout << "[ViPrescreen] Add: " << mem->GetID() << ", (seq: " << sequence_ << ")" << std::endl;
#endif
...
#ifdef VERSION_INDEX
void WriteBatch::AddViContext(ViContext* ctx) {
  ctx_.push_back(ctx);
}

void WriteBatch::PrepareUpdateXmax(std::unordered_map<std::string, int>& fds) {
  for (auto& ctx: ctx_) {
    if (!ctx->IsUserTable()) continue;
    switch (ctx->loc) {
      case ViContext::ViTargetLocation::NONE:
        // Skip this since the ctx wasn't updated by a put call
        continue;
      case ViContext::ViTargetLocation::MEMTABLE:
        break;
      case ViContext::ViTargetLocation::SST:
        {
          auto it = fds.find(ctx->fname);
          if (it != fds.end()) continue;

          // Open two file descriptors (.sst, .vi) and save it for use.
          int fd = open(ctx->fname.c_str(), O_RDWR, (mode_t)0600);
          if (fd >= 0) {
            // This means that the file has not been deleted.
            fds.insert(std::make_pair(ctx->fname, fd));

            std::string vi_fname = ctx->fname + ".vi";
            int vi_fd = open(vi_fname.c_str(), O_RDWR, (mode_t)0600);
            if (vi_fd >= 0) {
              fds.insert(std::make_pair(vi_fname, vi_fd));
            }
          }
        }
        break;
...
void WriteBatch::RunUpdateXmax(std::unordered_map<std::string, int>& fds,
                               const SequenceNumber seq) {
  char buf[8]; // buffer for xmax (i.e., seq_used)
  ...
  for (auto& ctx: ctx_) {
    ...
    switch (ctx->loc) {
      ...
      case ViContext::ViTargetLocation::MEMTABLE:
        {
          UpdateXmax(fds, ctx->key, ctx->mem_id, ctx->address, buf);
        }
        break;
      case ViContext::ViTargetLocation::SST:
        {
          UpdateXmax(fds, ctx->key, ctx->fname, ctx->off, buf);
        }
        break;
      default:
        // Should never reach here.
...
void WriteBatch::CleanupUpdateXmax(std::unordered_map<std::string, int>& fds) {
  for (auto& it: fds) {
    close(it.second);
  }
}

void WriteBatch::UpdateXmax(std::unordered_map<std::string, int>& fds,
                            const Slice& key,
                            const uint64_t mem_id,
                            char* address,
                            const char* seq) {
  assert(key.size() >= kNumInternalBytes);

  WriteXmaxForMemTable(fds, key, mem_id, address, seq);

  __sync_synchronize();

  memtable_dependency_mutex.Lock();
  auto dep_iter = memtable_dependency.find(mem_id);
  bool flush_exists = (dep_iter != memtable_dependency.end());
  memtable_dependency_mutex.Unlock();
  CompactionDependency* dep;
  
  // The concurrency issue here is described in details on the overloaded
  // UpdateXmax() function below. Please read it thoroughly as this part
  // is the one of the most important logic to ensure correctness.
  if (flush_exists) {
vi_flush_exists:
    dep = dep_iter->second;
    assert(dep != nullptr);
    ...
    while (dep->is_done.load(std::memory_order_relaxed) == false) {
      std::this_thread::yield();
    }

    /* The number of flush output should be 1 (i.e., memtable -> SST) */
    assert(dep->outputs.size() == 1);
    for (auto& output: dep->outputs) {
      UpdateXmaxRecursive(fds, key, output, seq);
    }
  } else {
    memtable_dependency_mutex.Lock();

    dep_iter = memtable_dependency.find(mem_id);
    flush_exists = (dep_iter != memtable_dependency.end());

    if (flush_exists) {
      memtable_dependency_mutex.Unlock();
      goto vi_flush_exists;
    }
...
void WriteBatch::UpdateXmax(std::unordered_map<std::string, int>& fds,
                            ...
                            const uint32_t xmax_offset,
                            const char* seq) {
  assert(key.size() >= kNumInternalBytes);

  WriteXmaxForSST(fds, key, fname, xmax_offset, seq); 

  __sync_synchronize();

  // Find files to write recursively
  uint64_t fnum = TableFileNameToNumber(fname);

  // TODO: check if lock is necessary for find.
  // According to the C++ standard, iterators are not effected by map
  // modifications. It is safe to just use find at this level, but use a lock
  // when you re-check on the if statement below.
  sst_dependency_mutex.Lock();
  auto dep_iter = sst_dependency.find(fnum);
  bool compaction_exists = (dep_iter != sst_dependency.end());
  sst_dependency_mutex.Unlock();
  CompactionDependency* dep;
  
  if (compaction_exists) {
vi_compaction_exists1:
    // We are sure that a compaction exists/existed.
    dep = dep_iter->second;
    assert(dep != nullptr);
    ...
    // Done with compaction, write recursively
    assert(dep->outputs.size() > 0);
    for (auto& output: dep->outputs) {
      UpdateXmaxRecursive(fds, key, output, seq);
    }
  } else {
    // It seems like there's no compaction. We must recheck after holding lock.
    //
    // TODO: Seems like a big lock that blocks ANY compaction from occuring
    // instead of the blocking compactions with the current file involved.
    // Check CompactionJob::Prepare() and CompactionJob::Install()
    sst_dependency_mutex.Lock();

    // Search again, since compaction might have been triggered before we
    // hold the above lock.
    dep_iter = sst_dependency.find(fnum);
    compaction_exists = (dep_iter != sst_dependency.end());

    if (compaction_exists) {
      // This means that a compaction has been initialized in the term before
      // we held the lock. Thus, we must wait until the compaction is done.
      sst_dependency_mutex.Unlock();
      goto vi_compaction_exists1;
    }

    // If we get here, we are sure that there wasn't any compaction before
    // the write of xmax in the input file.
    sst_dependency_mutex.Unlock();
  }
}

void WriteBatch::UpdateXmaxRecursive(std::unordered_map<std::string, int>& fds,
                                     const Slice& key,
                                     const uint64_t sst_num,
                                     const char* seq) {
  bool do_recursive = FindAndWriteXmaxForSST(fds, key, sst_num, seq);

  __sync_synchronize();

  // The key is not found in the existing file sst_num.
  // Other compaction files will include it which is also inside the compaction
  // dependency. Just skip the recursive part.
  if (!do_recursive) return;

  // TODO: The logic here is the same as UpdateXmax(). Combine it later.
  sst_dependency_mutex.Lock();
  auto dep_iter = sst_dependency.find(sst_num);
  bool compaction_exists = (dep_iter != sst_dependency.end());
  sst_dependency_mutex.Unlock();
  CompactionDependency* dep;
  
  if (compaction_exists) {
vi_compaction_exists2:
    dep = dep_iter->second;
    assert(dep != nullptr);
    ...
    dep_iter = sst_dependency.find(sst_num);
    compaction_exists = (dep_iter != sst_dependency.end());

    if (compaction_exists) {
      sst_dependency_mutex.Unlock();
      goto vi_compaction_exists2;
    }
...
void WriteBatch::WriteXmaxForMemTable(std::unordered_map<std::string, int>& fds,
                                      const Slice& key,
                                      const uint64_t mem_id,
                                      char* address,
                                      const char* seq) {
  // Check if memtable is alive and write xmax to the address
  live_memtables_mutex.Lock();
  auto iter = live_memtables.find(mem_id);
#ifdef VERSION_INDEX_DEBUG_XMAX_WRITER
  uint32_t user_key = Decode4ByteKey(&key.data()[4]);
  Slice memtable_key(address + 12 - key.size(), key.size());
#endif
  if (iter != live_memtables.end()) {
    // Key should be the same as what we have seen until xmin.
    assert(memcmp(key.data(), address + 12 - key.size(), key.size() - kNumInternalBytes + 8) == 0);
    memcpy(address, seq, sizeof(SequenceNumber));
    assert(DecodeFixed64(address) == DecodeFixed64(seq));
#ifdef VERSION_INDEX_DEBUG_XMAX_WRITER
    std::cout << "[XmaxWriter] Write to Mem (Success): " << mem_id << ", "
              << user_key << ", " << (void*)address
              << " (seq: " << DecodeFixed64(address) << "), "
              << ExtractXmin(memtable_key) << "~" << ExtractXmax(memtable_key) << std::endl;
#endif
  } else {
    // For debugging
#ifdef VERSION_INDEX_DEBUG_XMAX_WRITER
    std::cout << "[XmaxWriter] Write to Mem (Fail): " << mem_id << ", "
              << user_key << ", " << (void*)address
              << " (seq: " << DecodeFixed64(seq) << ")" << std::endl;
#endif
  }
  live_memtables_mutex.Unlock();
}

bool WriteBatch::FindAndWriteXmaxForSST(std::unordered_map<std::string, int>& fds,
                                        const Slice& key,
                                        const uint64_t sst_num,
                                        const char* seq) {
  // Find and write my xmax first.
  Options option;
  ReadOptions ropts;
  Status s;

  // Find the key + xmin inside the SST file.
  // We need to update the xmax there.
  SstFileReader reader(option);

  std::string sst_name = MakeTableFileName("./.rocksdb", sst_num);
  s = reader.Open(sst_name);

  if (!s.ok()) {
    // File must be deleted, still must try recursive so return true
    return true;
  }
  ...
  Status pik_status = ParseInternalKey(key, &ikey, false);
  for (iter->SeekToFirst(); iter->Valid(); iter->Next()) {
    // Compare key and xmin by checking the user key + 8 byte
    if (table_iter->CompareKeyWithXmin(key) == 0) {
#ifdef VERSION_INDEX_DEBUG
      int tmp_fd = open(sst_name.c_str(), O_RDWR, (mode_t)0600);
      if (tmp_fd >= 0) {
        char buf[key.size() - kNumInternalBytes + sizeof(SequenceNumber)];
        size_t ret = pread(tmp_fd, buf, sizeof(buf), table_iter->Offset());
        assert(ret == sizeof(buf));
        assert(memcmp(buf, key.data(), sizeof(buf)) == 0);
        close(tmp_fd);
      }
#endif
      found = true;
...
#if 0
  // TODO: delete, saved in case table_iter malfunctions 
  {
    if (iter->key().compare(ikey.user_key) == 0) {
      SequenceNumber xmin = ExtractXmin(table_iter->key());
      SequenceNumber xmax = ExtractXmax(table_iter->key());
      if (xmax == kMinUnCommittedSeq) {
        // Due to zeroing of bottom most level.
        if (xmin == ikey.sequence || xmin == 0) {
          found = true;
          break;
        }
      }
  ...
  if (found) {
    ...
    uint32_t offset = table_iter->Offset();
    uint32_t xmax_offset = offset + key.size() - kNumInternalBytes +
                              sizeof(SequenceNumber);

    WriteXmaxForSST(fds, key, sst_name, xmax_offset, seq);
  } else {
    // No need to continue from here since the overlapping key will be found in
    // another file, or the key might be deleted.
  }

  return found;
}

void WriteBatch::WriteXmaxForSST(std::unordered_map<std::string, int>& fds,
                                 const Slice& key,
                                 const std::string& fname,
                                 const uint32_t xmax_offset,
                                 const char* seq) {
  auto iter = fds.find(fname);
  std::string vi_fname = fname + ".vi";
  int fd, vi_fd;
  if (iter != fds.end()) {
    fd = iter->second;
  } else {
    // Open the .sst file
    fd = open(fname.c_str(), O_RDWR, (mode_t)0600);
    if (fd > 0) {
      fds.insert(std::make_pair(fname, fd));
    } else {
      // This means that the file has been deleted. (no need to update)
#ifdef VERSION_INDEX_DEBUG_XMAX_WRITER
      std::cout << "[XmaxWriter] Write to SST (Fail): " << fname << ", "
                << Decode4ByteKey(&key.data()[4]) << ", "
                << xmax_offset << " (seq: " << DecodeFixed64(seq) << ")" << std::endl;
#endif
      return;
    }
  }
  ...
  iter = fds.find(vi_fname);
  if (iter != fds.end()) {
    vi_fd = iter->second;
  } else {
    vi_fd = open(vi_fname.c_str(), O_RDWR, (mode_t)0600);
    if (vi_fd > 0) {
      fds.insert(std::make_pair(vi_fname, vi_fd));
    } else {
      // This means that the file has been deleted. (no need to update)
#ifdef VERSION_INDEX_DEBUG_XMAX_WRITER
      std::cout << "[XmaxWriter] Write to VI (Fail): " << vi_fname
                << " (seq: " << DecodeFixed64(seq) << ")" << std::endl;
#endif
      return;
    }
...
#ifdef VERSION_INDEX_DEBUG
  // Check if the content of xmax in SST file is the same as we expect.
  char buf[key.size()];
  pread(fd, buf, sizeof(buf), xmax_offset + 12 - key.size());
  assert(memcmp(buf, key.data(), key.size() - kNumInternalBytes) == 0);
  SequenceNumber prev_xmax = DecodeFixed64(buf + key.size() - 12);
  assert((prev_xmax == DecodeFixed64(seq)) || (prev_xmax == kMinUnCommittedSeq));
#endif

  // Write to .sst file
  ssize_t written = pwrite(fd, seq, sizeof(SequenceNumber), xmax_offset);
  assert(written == sizeof(SequenceNumber));

#ifdef VERSION_INDEX_DEBUG_XMAX_WRITER
    std::cout << "[XmaxWriter] Write to SST (Success): " << fname << ", "
              << Decode4ByteKey(&key.data()[4]) << ", " << xmax_offset
              << " (seq: " << DecodeFixed64(seq) << ")" << std::endl;
#endif

  // Write to .vi file
  // xmax is after xmin in the version index (verified)
  // TODO: currently read from SST file. There might be a better way.
  char vi_buf[4];
  int ret = pread(fd, vi_buf, sizeof(vi_buf), xmax_offset + 8);
  assert(ret == 4);
  uint32_t vi_xmax_offset = DecodeFixed32(vi_buf) + 8;
  written = pwrite(vi_fd, seq, sizeof(SequenceNumber), vi_xmax_offset);
  assert(written == sizeof(SequenceNumber));

#ifdef VERSION_INDEX_DEBUG_XMAX_WRITER
    std::cout << "[XmaxWriter] Write to VI (Success): " << vi_fname << ", "
      << vi_xmax_offset << " (seq: " << DecodeFixed64(seq) << ")" << std::endl;
#endif
}
#endif
