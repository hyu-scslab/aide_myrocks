...
#ifdef SMARTSSD
int Version::CountSstFiles() {
  int sst_files_count = 0;
  for (int level = 0; level < storage_info_.num_levels_; level++) {
    sst_files_count += storage_info_.NumLevelFiles(level);
  }
  return sst_files_count;
}

void Version::GetFiles(std::vector<std::string> *files) {
  // TODO: file names for mem and imm
  for (int level = 0; level < storage_info_.num_levels_; level++) {
    for (const auto& f : storage_info_.LevelFiles(level)) {
      files->push_back(TableFileName(cfd()->ioptions()->cf_paths, 
            f->fd.GetNumber(), f->fd.GetPathId()).c_str());
    }
  }
}
#endif //SMARTSSD
...
void Version::PrintRef() {
  ++refs_;
  fprintf(stderr, "Cur ref : %d\n", refs_);
}
...
/* Get all version file name for AIDE */
void Version::GetAllSstFileName(
    const ImmutableOptions& ioptions,
    std::vector<std::vector<std::string> * >& fname_vector) {
  std::vector<std::string> * cf_fname_vector = new std::vector<std::string>();
  /* read all file levels in superversion */
  for (int level = 0; level < storage_info_.num_levels_; level++) {
    for (const auto& file_meta : storage_info_.LevelFiles(level)) {
      /* make file name and push return vector */
      cf_fname_vector->push_back(TableFileName(cfd()->ioptions()->cf_paths,
        file_meta->fd.GetNumber(), file_meta->fd.GetPathId()).c_str());
			fprintf(stderr, "%s\n", 
					TableFileName(cfd()->ioptions()->cf_paths, file_meta->fd.GetNumber(),
							file_meta->fd.GetPathId()).c_str());
    }
  }
	fname_vector.push_back(cf_fname_vector);
}
#endif
...
void VersionSet::GetObsoleteFiles(std::vector<ObsoleteFileInfo>* files,
                                  ...
                                  std::list<uint64_t>& aide_pending_outputs)
#else
void VersionSet::GetObsoleteFiles(std::vector<ObsoleteFileInfo>* files,
                                  std::vector<ObsoleteBlobFileInfo>* blob_files,
                                  std::vector<std::string>* manifest_filenames,
...
#endif
{
  ...
  for (auto& f : obsolete_files_) {
    ...
    if (f.metadata->fd.GetNumber() < min_pending_output || 
        find(aide_pending_outputs.begin(), aide_pending_outputs.end(), 
             f.metadata->fd.GetNumber()) != aide_pending_outputs.end())
#else
    if (f.metadata->fd.GetNumber() < min_pending_output)
#endif
    {
  ...
  for (auto& blob_file : obsolete_blob_files_) {
    ...
    if (blob_file.GetBlobFileNumber() < min_pending_output ||
        find(aide_pending_outputs.begin(), aide_pending_outputs.end(), 
             blob_file.GetBlobFileNumber()) != aide_pending_outputs.end()) 
#else
    if (blob_file.GetBlobFileNumber() < min_pending_output)
#endif
    {
