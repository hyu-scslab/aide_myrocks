...
void SstFileManagerImpl::OnDeleteFileImpl(const std::string& file_path) {
  ...
  if (file_path.find(".sst") != std::string::npos) {
    std::string vi_file_path = file_path + ".vi";
    std::string i_file_path = file_path + ".i";
    Status vi_s;
    IOOptions option;

    vi_s = fs_->FileExists(vi_file_path, option, nullptr);

    if (vi_s.ok())
    {
      vi_s = fs_->DeleteFile(vi_file_path, IOOptions(), nullptr);
      assert(vi_s.ok());
    }

    vi_s = fs_->FileExists(i_file_path, option, nullptr);

    if (vi_s.ok())
    {
      vi_s = fs_->DeleteFile(i_file_path, IOOptions(), nullptr);
      assert(vi_s.ok());
    }

    uint64_t fnum = TableFileNameToNumber(file_path);

    // Remove from sst_dependency
    sst_dependency_mutex.Lock();
    auto iter = sst_dependency.find(fnum);
    if (iter != sst_dependency.end()) {
