...
#ifdef VERSION_INDEX
  if (IsUserTable(column_family->GetID())) {
    WriteBatchWithIndex* wbwi = GetWriteBatch();
    assert(wbwi);
    WriteBatch* wb = wbwi->GetWriteBatch();
    assert(wb);
    assert(vi_ctx == nullptr);
    vi_ctx = new ViContext(column_family->GetID());
    wb->AddViContext(vi_ctx);
  }
#endif
  Status s = GetForUpdateImpl(read_options, column_family, key, value,
...
#endif
  Status s = GetForUpdateImpl(read_options, column_family, key, pinnable_val,
                              exclusive, do_validate);
#ifdef VERSION_INDEX
  if (IsUserTable(column_family->GetID())) {
    // If the current GetForUpdate found a match, it would be successfully
    // appended to the WriteBatch::ctx_. Also, the cleanup of the object
    // is the write batch's job to do.
    vi_ctx = nullptr;
  }
#endif
  return s;
...
Status WriteCommittedTxn::CommitWithoutPrepareInternal() {
  ...
  if (s.ok()) {
    SetId(seq_used);
#ifdef VERSION_INDEX
    std::unordered_map<std::string, int> vi_fds;

    // Phase 1. prepare
    wb->PrepareUpdateXmax(vi_fds);

    // Phase 2. install xmax
    wb->RunUpdateXmax(vi_fds, seq_used);

    // Phase 3. cleanup
    wb->CleanupUpdateXmax(vi_fds);
#endif
  }
  return s;
}
...
Status WriteCommittedTxn::CommitBatchInternal(WriteBatch* batch, size_t) {
  ...
  if (s.ok()) {
    ...
    // Phase 1. prepare
    batch->PrepareUpdateXmax(vi_fds);

    // Phase 2. install xmax
    batch->RunUpdateXmax(vi_fds, seq_used);

    // Phase 3. cleanup
    batch->CleanupUpdateXmax(vi_fds);
#endif
  }
  return s;
}
