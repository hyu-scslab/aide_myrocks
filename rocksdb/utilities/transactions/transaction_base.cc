...
Status TransactionBaseImpl::FlushRequestForAIDE(
    ColumnFamilyHandle* column_families,
    SequenceNumber visible_seq_no,
  ...
  return db_->FlushRequestForAIDE(column_families, visible_seq_no, 
            earliest_mem_ids, latest_mem_ids, num_flush, wait_cfd, wait_memid,
            wait_file_snapshots, pending_output_files, fname_vector);
}

Status TransactionBaseImpl::FlushRequestForAIDE(
    const std::vector<ColumnFamilyHandle*>& column_families,
    const std::vector<SequenceNumber>& visible_seq_nos,
    std::vector<uint64_t>& earliest_mem_ids,
    std::vector<uint64_t>& latest_mem_ids,
    std::vector<int>& num_flush,
    std::vector<void*>& wait_cfd,
 ...
 return db_->FlushRequestForAIDE(column_families, visible_seq_nos, 
            earliest_mem_ids, latest_mem_ids, num_flush, wait_cfd, wait_memid,
            wait_file_snapshots, pending_output_files, fname_vector);
}

Status TransactionBaseImpl::WaitForFlushVisibleMemTables(
      const std::vector<void*>& wait_cfd,
      std::vector<uint64_t>& wait_memid,
      std::vector<std::vector<std::string>*>& fname_vector,
      std::vector<uint64_t>& earliest_memids,
      std::vector<uint64_t>& latest_memids) {
  return db_->WaitForFlushVisibleMemTables(wait_cfd, wait_memid,
            fname_vector, earliest_memids, latest_memids);
}
...
Status TransactionBaseImpl::WaitForFlushVisibleMemTables(
  ...
  return db_->WaitForFlushVisibleMemTables(wait_cfd, wait_memid,
            fname_vector, earliest_memids, latest_memids, stall_file_deletes);
}
void TransactionBaseImpl::ReleaseAideInfo(
    std::vector<void*>& wait_file_snapshot,
    std::vector<std::vector<std::string>*>& fname_vector,
    std::vector<uint64_t>* stall_file_deletes){
  db_->ReleaseAideInfo(wait_file_snapshot, fname_vector, stall_file_deletes);
}
#endif
