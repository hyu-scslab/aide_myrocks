...
class WriteBatch : public WriteBatchBase {
...
#ifdef VERSION_INDEX
  void AddViContext(ViContext* ctx);

  void PrepareUpdateXmax(std::unordered_map<std::string, int>& fds);

  void RunUpdateXmax(std::unordered_map<std::string, int>& fds,
                     const SequenceNumber seq);

  void CleanupUpdateXmax(std::unordered_map<std::string, int>& fds);

#ifdef VERSION_INDEX_DEBUG
  size_t ViContextSize() { return ctx_.size(); }

  ViContext* GetFirst() { return ctx_[0]; }
#endif
#endif
 ...
 private:
...
#ifdef VERSION_INDEX
  // For MemTable
  void UpdateXmax(std::unordered_map<std::string, int>& fds,
                  const Slice& key,
  ...
  // REQUIRED: sst_num file should be immutable, except xmax.
  void UpdateXmaxRecursive(std::unordered_map<std::string, int>& fds,
                           const Slice& key,
                           const uint64_t sst_num,
                           const char* seq);

  void WriteXmaxForMemTable(std::unordered_map<std::string, int>& fds,
                            const Slice& key,
                            const uint64_t mem_id,
                            char* address,
                            const char* seq);

  void WriteXmaxForSST(std::unordered_map<std::string, int>& fds,
                       const Slice& key,
                       const std::string& fname,
                       const uint32_t xmax_offset,
                       const char* seq);

  // Returns true if there's a matching key, or the file doesn't exist.
  // In case of file not exist, we still must go through the recursive case.
  bool FindAndWriteXmaxForSST(std::unordered_map<std::string, int>& fds,
                              const Slice& key,
                              const uint64_t sst_num,
                              const char* seq);

  std::vector<ViContext*> ctx_;
#endif
