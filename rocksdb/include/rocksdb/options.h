...
struct ReadOptions {
  ...
  // s3d
  bool s3d_scan;
  uint s3d_buffer_size; /* The amount to fetch from FPGA once, currently 4MB */
  uint s3d_total_buffer_read; /* The number of time to read from FPGA */
  uint s3d_page_size; /* The internal page size of rocksdb, currently 16K */
#endif
#ifdef SMARTSSD_FLUSH_MEMTABLE
  uint16_t ndp_key_size; /* packed key size used by NDP */
#endif
