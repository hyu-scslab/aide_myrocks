...
bool JOIN::set_access_methods()
{
...
#ifdef SMARTSSD
    uint s3d_tab_to_set = 0, s3d_tab_cur = 0;
    if (s3d_target && thd->variables.s3d_forced_table_scan) {
      s3d_tab_to_set = 1 + ((s3d_max_num_of_joins - 2) / 2) + 1;
    }
#endif
  full_join= false;
  ...
  for (uint tableno= const_tables; tableno < tables; tableno++)
  {
...
#ifdef SMARTSSD
    Key_use *const keyuse= (s3d_target && thd->variables.s3d_forced_table_scan && 
      (s3d_tab_cur++ < s3d_tab_to_set)) ? NULL : tab->position->key;
#else
    ...
    else if (!keyuse)
    {
      ...
      if (s3d_target && thd->variables.s3d_forced_table_scan) {
        fprintf(stderr, "[SMARTSSD] table:%s, JT_ALL, non-const:%d, total:%u, cur:%u, %d %s\n",
          tab->table->s->table_name.str, (tableno > const_tables), s3d_tab_to_set, s3d_tab_cur,
          __LINE__, __func__); 
      }
#endif
    }
...
make_join_readinfo(JOIN *join, ulonglong options, uint no_jbuf_after)
{
  ...
  for (uint i= join->const_tables; i < join->tables; i++)
  {
	...
	{
#ifdef SMARTSSD
          if (!(join->s3d_target && join->thd->variables.s3d_forced_table_scan)) {
#endif
          if (tab->select && tab->select->quick &&
