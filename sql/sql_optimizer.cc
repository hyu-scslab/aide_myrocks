...
static const char *ndp_src_table_names[]= {
  NULL,       /*  0 */
  "sbtest1",  /*  1 */
  "sbtest2",  /*  2 */
  "sbtest3",  /*  3 */
...
static ha_rows get_quick_record_count(THD *thd, SQL_SELECT *select,
              TABLE *table, const key_map *keys,ha_rows limit);
...
#ifdef SMARTSSD_S3D_CONDS_DEBUG
void print_conds(Item* where, const char* file, int line) {
  int i, idx;
  Item* item;
  Item_field* arg;
  fprintf(stderr, "[SMARTSSD] JOIN::optimize conds: %p at %s(%d)\n", where, file, line);
  fprintf(stderr, "-----------------------------------------------------------\n");
  for (item = where, idx = 0; item; item = item->next, idx++) {
    switch (item->type()) {
      /*
      case Item::FIELD_ITEM:
        fprintf(stderr, "[SMARTSSD] item[%d]: %p item->type: %d\n", idx, item, item->type());
        break;
        */
      case Item::FUNC_ITEM:
        fprintf(stderr, "[SMARTSSD] item[%d]: %p item->type: %d\n", idx, item, item->type());
        fprintf(stderr, "[SMARTSSD] - item->functype: %d (%s, argc: %d arg: %p, %p)\n", 
            ((Item_cond*) item)->functype(),
            ((Item_func*) item)->func_name(),
            ((Item_func*) item)->arg_count,
            ((Item_func*) item)->arg_count > 0 ?  ((Item_func*) item)->args[0] : 0,
            ((Item_func*) item)->arg_count > 1 ?  ((Item_func*) item)->args[1] : 0);
        for (i = 0; i < ((Item_func*) item)->arg_count; i++) {
          arg = (Item_field*) ((Item_func*) item)->args[i];
          switch (arg->type()) {
            case Item::FIELD_ITEM:
              fprintf(stderr, "[SMARTSSD] - args[%d] %s.%s(%d)\n", i,
                  ((Item_field*) arg)->cached_table->table_name,
                  ((Item_field*) arg)->field->field_name,
                  ((Item_field*) arg)->field->field_index);
              break;
            case Item::STRING_ITEM:
              fprintf(stderr, "[SMARTSSD] - args[%d] value: %s len: %d max: %u\n", i,
                  ((Item_string*) arg)->str_value.ptr(),
                  ((Item_string*) arg)->str_value.length(),
                  ((Item_string*) arg)->max_length);
              break;
            case Item::INT_ITEM:
              fprintf(stderr, "[SMARTSSD] - args[%d] value: %lld len: %u\n", i,
                  ((Item_int*) arg)->value,
                  ((Item_int*) arg)->max_length);
              break;
            default:
              fprintf(stderr, "[SMARTSSD] - args[%d] ??? type: %d\n", i, arg->type());
              /*
              if (
              fprintf(stderr, "[SMARTSSD] - args[%d] value: \n", i,
                  ((Item_field*) item)->table_name, ((Item_field*) item)->field_name,
                  ((Item_field*) item)->cached_field_index);
                  */
              break;
          }
        }
      ...
      case Item::COND_ITEM:
        fprintf(stderr, "[SMARTSSD] item[%d]: %p item->type: %d\n", idx, item, item->type());
        fprintf(stderr, "[SMARTSSD] - item->functype: %d cond: %s\n", 
            ((Item_cond*) item)->functype(),
            ((Item_func*) item)->func_name());
        break;
      default:
...
#else 
#define print_conds(where, file, line) do {} while (0)
#endif //SMARTSSD
...
JOIN::optimize()
{
...
#ifdef SMARTSSD
  Item* s3d_conds;
  print_conds(conds, __FILE__, __LINE__);
#endif //SMARTSSD
...
#ifdef SMARTSSD
  /*
  if (conds && !(select_options & SELECT_DESCRIBE))
  {
    uint num_tab, len;
    void* s3d_join_tab;

    if (thd->variables.ndp_src_table != NDP_SRC_NONE) {
      s3d_target_table_name = ndp_src_table_names[thd->variables.ndp_src_table];
      uint _s3d_max_num_of_joins = thd->variables.ndp_max_joins;
      s3d_join_tab = make_s3d_join_tab(conds, &len);

      if (len) {
        s3d_target_list_len = len;
        s3d_target->file->s3d_join_tab_cand = s3d_join_tab;

        num_tab = (_s3d_max_num_of_joins == 0 
            || s3d_target_list_len < _s3d_max_num_of_joins)
          ? s3d_target_list_len : _s3d_max_num_of_joins;

        if (num_tab) {
          s3d_target->file->s3d_first_table_cand = true;
          s3d_target->file->s3d_len_join_tab = num_tab; //s3d_target_list_len;
        }
      }
    }
  }
...
#ifdef SMARTSSD
  if (s3d_conds && !(select_options & SELECT_DESCRIBE))
  {
    uint num_tab, len;
    void* s3d_join_tab;
    ...
    if (thd->variables.ndp_src_table != NDP_SRC_NONE) {
      s3d_target_table_name = ndp_src_table_names[thd->variables.ndp_src_table];
      uint _s3d_max_num_of_joins = thd->variables.ndp_max_joins;
      s3d_join_tab = make_s3d_join_tab(s3d_conds, &len);
...
#ifdef SMARTSSD
  if (s3d_target && !(select_options & SELECT_DESCRIBE))
  {
    JOIN_TAB* t;
    S3D_JOIN_TAB* s;
    uint i, num_tab;
    num_tab = s3d_target->file->s3d_len_join_tab;

    for (t = join_tab; t < join_tab + primary_tables; t++) {
      for (s = (S3D_JOIN_TAB*) s3d_target->file->s3d_join_tab_cand, i = 0;
          s && i < num_tab; s++, i++) {
        if (!strcmp(t->table->s->table_name.str, s->table_name)) {
          // Reset full normalized table name for future use
          fprintf(stderr, "Set table[%d]: %p (%s)\n", 
              i, s, t->table->s->table_name.str);
          s->table = t->table;
          t->table->file->s3d_join_tab_cand = s3d_target->file->s3d_join_tab_cand;
          /*
          if (!t->table->file->s3d_join_tab_cand)
            t->table->file->s3d_join_tab_cand = s3d_target->file->s3d_;
            */
        }
      }
    }
  ...
  */
  ...
  {
#ifdef SMARTSSD
    pick_table_access_method (&join_tab[i], s3d_target);
#else
...
#ifdef SMARTSSD
S3D_JOIN_TAB* JOIN::make_s3d_join_tab(Item* where, uint* len)
{
  unsigned int i;
  int j, s3d_tab_len;
  Item *item;
  Item_field *arg, *val;
  Item_func* i_func;
  S3D_JOIN_TAB *s3d_tab, *tab_list;
  List_iterator<Item> li;

  /* 
     First, find join pair for each condition. Here, we assume that 
     all condition are connected with AND.
     */
  /*
  if (where->type() == Item::COND_ITEM) {
    li.init(((Item_cond*) where)->list);
    item = li++;
  } else {
  */
    item = where;
  ...
  j = s3d_tab_len = 0;
  while (item) {
#ifdef SMARTSSD_DEBUG_MAKE_S3D_JOIN_TAB
    fprintf(stderr, "[SMARTSSD] %uth item %p type: %d len: %i\n", 
        j++, item, item->type(), s3d_tab_len);
#endif //SMARTSSD_DEBUG_MAKE_S3D_JOIN_TAB
    if (item->type() == Item::FUNC_ITEM) {
      i_func = (Item_func*) item;
      if (i_func->arg_count == 2 
          && i_func->functype() == Item_func::EQ_FUNC
          && i_func->args[0]->type() == Item::FIELD_ITEM 
          && i_func->args[1]->type() == Item::FIELD_ITEM) {
        for (i = i_func->arg_count - 1; i < 2; i--, s3d_tab_len++) {
          if (s3d_target)
            continue;

          arg = (Item_field*) i_func->args[i];
          if (!strcmp(arg->cached_table->table_name, s3d_target_table_name)) {
#ifdef SMARTSSD_DEBUG_MAKE_S3D_JOIN_TAB
            fprintf(stderr, "[SMARTSSD]   found target table: %p\n", 
                arg->cached_table->table);
#endif //SMARTSSD_DEBUG_MAKE_S3D_JOIN_TAB
            s3d_target = arg->cached_table->table;
          }
        }
      }
    }
    //item = where->type() == Item::COND_ITEM ? li++ : item->next;
    item = item->next;
  }
#ifdef SMARTSSD_DEBUG_MAKE_S3D_JOIN_TAB
  fprintf(stderr, "[SMARTSSD]   set tab_len as %u\n", s3d_tab_len);
#endif //SMARTSSD_DEBUG_MAKE_S3D_JOIN_TAB
  ...
  //s3d_tab_len <<= 1;
  tab_list = new S3D_JOIN_TAB[s3d_tab_len]; 

  /* Build join pairs for given condition */
#if 0
  if (where->type() == Item::COND_ITEM) {
    li.init(((Item_cond*) where)->list);
    item = li++;
  ...
  s3d_tab = &tab_list[s3d_tab_len - 1]; 
  while (item) {
    if (item->type() == Item::FUNC_ITEM) {
      i_func = (Item_func*) item;
      if (i_func->arg_count == 2 
        ...
        for (i = 0; i < 2; i++) {
          ...
          s3d_tab->table_name = arg->cached_table->table_name;
          s3d_tab->table = NULL;
          s3d_tab->cfh = NULL;
          s3d_tab->idx = arg->field->field_index;
          s3d_tab->op_type = NDP_FILTER_NONE;
          s3d_tab--;
        }
      }
    }
  ...
  /* Find filter conditions for each join pair */
  if (where->type() == Item::COND_ITEM) {
    li.init(((Item_cond*) where)->list);
    item = li++;
  } else {
  ...
  while (item) {
    if (item->type() == Item::FUNC_ITEM) {
      ...
      if (i_func->arg_count == 2 
          && (i_func->args[1]->type() != Item::FIELD_ITEM 
            || i_func->args[0]->type() != Item::FIELD_ITEM)) {
        for (i = 0; i < i_func->arg_count; i++) {
          arg = (Item_field*) i_func->args[i];
          if (arg->type() == Item::FIELD_ITEM) {
            for (j = 0; j < s3d_tab_len; j++) {
              s3d_tab = &tab_list[j];
              if (!strcmp(s3d_tab->table_name, arg->cached_table->table_name)
                  && s3d_tab->op_type == NDP_FILTER_NONE) {
                switch (i_func->functype()) {
                  case Item_func::EQ_FUNC:
                    s3d_tab->op_type = NDP_FILTER_EQ;
                    break;
                  case Item_func::LT_FUNC:
                    s3d_tab->op_type = NDP_FILTER_LT;
                    break;
                  case Item_func::LE_FUNC:
                    s3d_tab->op_type = NDP_FILTER_LTE;
                    break;
                  case Item_func::GE_FUNC:
                    s3d_tab->op_type = NDP_FILTER_GTE;
                    break;
                  case Item_func::GT_FUNC:
                    s3d_tab->op_type = NDP_FILTER_GT;
                    break;
                  case Item_func::LIKE_FUNC:
                    s3d_tab->op_type = NDP_FILTER_EQ;
                    break;
                }
                s3d_tab->target_idx = arg->field->field_index;
                val = (Item_field*) i_func->args[!i];

                switch (val->type()) {
                  case Item::STRING_ITEM:
                    s3d_tab->attr_type = NDP_FILTER_STRING;
                    {
                      String* str = &((Item_string*) val)->str_value;
                      const char* str_val = str->ptr();
                      s3d_tab->prefix_len = str->length() < 8 ? str->length() : 8;
                      if (str_val[s3d_tab->prefix_len - 1] == '%') {
                        s3d_tab->prefix_len--;
                      }
                      memcpy(&s3d_tab->operand, str_val, s3d_tab->prefix_len);
                    }
                    break;
                  case Item::INT_ITEM:
                    s3d_tab->attr_type = NDP_FILTER_4B;
                    s3d_tab->operand = ((Item_int*) val)->value;
                    s3d_tab->prefix_len = arg->field->b_len;
                    break;
                }
              }
            }
  ...
  //FPGAManager_GetInstance(s3d_kernel_name_ptr, true);
  //NDPObject_GetInstance(node->state->es_fpga_mgr, (void *)message, "");

  //DBUG_RETURN((void*) tab);
  *len = s3d_tab_len;
  return tab_list;
}
#endif  // SMARTSSD
...
#ifdef SMARTSSD
    if (join->s3d_target && join->s3d_target_table_name &&
      !strcmp(table->s->table_name.str, join->s3d_target_table_name)) 
    {
      //join->s3d_target = table;
      s3d_stats_records = table->file->stats.records;
      table->file->stats.records = 2;	    
    }
#endif
...
#ifdef SMARTSSD
  if (join->s3d_target != NULL)
  {
    join->s3d_target->file->stats.records = s3d_stats_records;
  }
#endif
        ...
        join->thd->optimizer_switch_flag(OPTIMIZER_SWITCH_MATERIALIZATION))
    {
      ...
      */
      ...
      memcpy(sj_nest->nested_join->sjm.positions,
             ...
             sizeof(struct st_position) * n_tables);
    }
  }
  DBUG_RETURN(false);
}
...
#if 0
	fprintf(stderr, "[SMARTSSD] optimized_cond thd: %p conds: %p at %s(%d)\n",
			thd, conds, __FILE__, __LINE__);
#endif
