...
#ifdef SMARTSSD
typedef struct st_s3d_join_tab {
  const char *table_name;
  TABLE* table;
  void* cfh; // place holder for ColumnFamilyHandle
...
#ifdef SMARTSSD
  uint s3d_target_list_len;
  TABLE* s3d_target;
  const char* s3d_target_table_name;
  S3D_JOIN_TAB* make_s3d_join_tab(Item* where, uint* len);
#endif
