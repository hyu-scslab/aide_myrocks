...
#ifdef SMARTSSD
  else if (select && select->quick && !table->file->s3d_join_tab_cand)
#else
...
#ifdef SMARTSSD
    if (table->file->s3d_join_tab_cand) {
#ifdef SMARTSSD_DEBUG    
      fprintf(stderr, "[SMARTSSD] using rr_sequential, %s, %d %s\n", 
          table->s->table_name.str, __LINE__, __func__);
#endif      
      if (table->file->s3d_first_table_cand)
        table->file->s3d_join_push();
    }
#endif
...
{ 
#ifdef SMARTSSD
  if (info->table && info->table->file->s3d_join_tab_cand) {
    if (info->table->file->s3d_first_table_cand) {
#ifdef SMARTSSD_DEBUG
      fprintf(stderr, "[SMARTSSD] delete s3d_join_tab, %s, %d %s\n", 
          info->table->s->table_name.str, __LINE__, __func__);
      delete info->table->file->s3d_join_tab_cand;    
      info->table->file->s3d_len_join_tab = 0;
      info->table->file->s3d_first_table_cand = false;
#endif
    }
    info->table->file->s3d_join_tab_cand = NULL;
  }
#endif
