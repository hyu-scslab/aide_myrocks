...
static Sys_var_mybool Sys_slave_skip_max_binlog_cache_size_check(
       ...
       GLOBAL_VAR(slave_skip_max_binlog_cache_size_check),
       CMD_LINE(OPT_ARG), DEFAULT(FALSE));
...
#ifdef SMARTSSD
// TODO: legacy code, must delete if no use.
static Sys_var_mybool Sys_ndp(
       "ndp", "Make scans take NDP path "
       "NDP",
       SESSION_VAR(ndp), CMD_LINE(OPT_ARG), DEFAULT(FALSE));

static Sys_var_ulong Sys_ndp_max_joins(
       "ndp_max_joins",
       "Maximum number of joins for NDP",
       SESSION_VAR(ndp_max_joins), CMD_LINE(OPT_ARG),
       VALID_RANGE(0, 1<<30), DEFAULT(0), BLOCK_SIZE(1));

/**
 * CAUTION:
 * This list is strictly matched with the enum value enum_ndp_src_table
 * in sql_class.h. If you add any other tables, make sure you add it in the
 * SAME order into both sides.
 *
 * How to check the source table in MyRocks:
 * if (thd->variables.ndp_src_table == NDP_SRC_NONE) // NONE
 * if (thd->variables.ndp_src_table == NDP_SRC_SBTEST1) // SBTEST1
 * if (thd->variables.ndp_src_table == NDP_SRC_SBTEST2) // SBTEST2
 *
 * ...
 *
 * If you add CH-benchmark tables, please append it to the below list
 * and use it like the above use cases.
 *
 * ex) if (thd->variables.ndp_src_table == NDP_SRC_STOCK) // STOCK
 *
 * Usage in mysql client:
 * mysql> SELECT @@session.ndp_src_table;
 * +-------------------------+
 * | @@session.ndp_src_table |
 * +-------------------------+
 * | NONE                    |
 * +-------------------------+
 * 1 row in set (0.00 sec)
 *
 * mysql> SET @@session.ndp_src_table="SBTEST1";
...
static Sys_var_enum Sys_ndp_src_table(
       "ndp_src_table",
       "Set source table for NDP join",
       SESSION_VAR(ndp_src_table), CMD_LINE(OPT_ARG),
       ndp_src_table_names, DEFAULT(NDP_SRC_NONE),
       NO_MUTEX_GUARD, NOT_IN_BINLOG);
#endif
...
#ifdef SMARTSSD
static Sys_var_charptr Sys_s3d_kernel_name(
       "s3d_kernel_name", "Kernel name to be invoked for in-situ filtering and join",
       READ_ONLY GLOBAL_VAR(s3d_kernel_name_ptr), CMD_LINE(OPT_ARG),
       IN_FS_CHARSET, DEFAULT(0), nullptr);

static Sys_var_charptr Sys_s3d_target_table(
       "s3d_target_table", "Table name whose filtering and join would be conducted in s3d",
       READ_ONLY GLOBAL_VAR(s3d_target_table_ptr), CMD_LINE(OPT_ARG),
       IN_FS_CHARSET, DEFAULT(0), nullptr);

static Sys_var_uint Sys_s3d_max_num_of_joins(
       "s3d_max_num_of_joins",
       "Max number of joins to be allowed for FPGA offloaing",
       READ_ONLY GLOBAL_VAR(s3d_max_num_of_joins), CMD_LINE(OPT_ARG),
       VALID_RANGE(0, UINT_MAX), DEFAULT(0), BLOCK_SIZE(1));

static Sys_var_mybool Sys_s3d_forced_table_scan(
       "s3d_forced_table_scan",
       "Force table scan for s3d offloading",
       SESSION_VAR(s3d_forced_table_scan),
       CMD_LINE(OPT_ARG), DEFAULT(FALSE));
#endif
