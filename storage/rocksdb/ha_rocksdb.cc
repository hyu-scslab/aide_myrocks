...
class Rdb_transaction {
...
#ifdef SMARTSSD_FLUSH_MEMTABLE
  /* file name vector for cfh */
  std::vector<void*> m_wait_file_snapshot;
  std::vector<uint64_t> m_pending_output_files;
  std::vector<std::vector<std::string>*> m_fname_vector;
#endif
  ...
  */
...
#ifdef SMARTSSD
      , uint16_t ndp_key_size = 28, /* sysbench key size */
      bool s3d_scan = false,
      uint s3d_buffer_size = 4194304, /* 4 MB */
      uint s3d_total_buffer_read = 0,
      uint s3d_page_size = 16284
#endif
    ) {
...
#ifdef SMARTSSD
    options.ndp_key_size = ndp_key_size;
    options.s3d_scan = s3d_scan;
    options.s3d_buffer_size = s3d_buffer_size;
    options.s3d_total_buffer_read = s3d_total_buffer_read;
    options.s3d_page_size = s3d_page_size;
#endif
    if (read_current) {
...
#ifdef SMARTSSD_FLUSH_MEMTABLE
  virtual int flush_request_for_aide_impl(
      rocksdb::ColumnFamilyHandle * column_family,
      rocksdb::SequenceNumber seq_no,
      std::vector<int>& num_flush,
      std::vector<void*>& wait_cfd,
      std::vector<uint64_t>& wait_memid,
      std::vector<uint64_t>& earliest_mem_ids,
      std::vector<uint64_t>& latest_mem_ids,
      std::vector<std::vector<std::string>*>& fname_vector) = 0;

  int flush_request_for_aide(rocksdb::ColumnFamilyHandle * column_family,
      rocksdb::SequenceNumber seq_no, std::vector<int>& num_flush,
      std::vector<void*>& wait_cfd, std::vector<uint64_t>& wait_memid,
      std::vector<uint64_t>& earliest_mem_ids,
      std::vector<uint64_t>& latest_mem_ids,
      std::vector<std::vector<std::string>*>& fname_vector) {
    return flush_request_for_aide_impl(column_family, seq_no, num_flush,
                                       wait_cfd, wait_memid,
                                       earliest_mem_ids, latest_mem_ids, 
                                       fname_vector);
  } 

  virtual int flush_request_for_aide_impl(
      std::vector<rocksdb::ColumnFamilyHandle*>& column_families,
      std::vector<rocksdb::SequenceNumber>& visible_seq_nos,
      std::vector<int>& num_flush,
      std::vector<void*>& wait_cfd,
      std::vector<uint64_t>& wait_memid,
      std::vector<uint64_t>& earliest_mem_ids,
  ...
  int flush_request_for_aide(
      ...
      std::vector<uint64_t>& latest_mem_ids,
      std::vector<std::vector<std::string>*>& fname_vector) {
    return flush_request_for_aide_impl(column_families, visible_seq_nos,
              num_flush, wait_cfd, wait_memid, earliest_mem_ids, latest_mem_ids, 
              fname_vector);
  }

  virtual int wait_flush_for_aide_impl(std::vector<void*>& wait_cfd,
      std::vector<uint64_t>& wait_memid,
      std::vector<uint64_t>& earliest_mem_ids,
      std::vector<uint64_t>& latest_mem_ids,
      std::vector<std::vector<std::string>*>& fname_vector) = 0;

  int wait_flush_for_aide(std::vector<void*>& wait_cfd,
      std::vector<uint64_t>& wait_memid,
      std::vector<uint64_t>& earliest_mem_ids,
      std::vector<uint64_t>& latest_mem_ids,
      std::vector<std::vector<std::string>*>& fname_vector) {
    return wait_flush_for_aide_impl(wait_cfd, wait_memid, earliest_mem_ids,
                                    latest_mem_ids, fname_vector);
  }

  virtual int wait_flush_for_aide_impl(void* wait_cfd, uint64_t wait_memid,
      uint64_t earliest_mem_ids, uint64_t latest_mem_ids,
      std::vector<std::string>* fname_vector) = 0;

  int wait_flush_for_aide(void* wait_cfd, uint64_t wait_memid,
      uint64_t earliest_mem_ids, uint64_t latest_mem_ids,
      std::vector<std::string>* fname_vector) {
    return wait_flush_for_aide_impl(wait_cfd, wait_memid, earliest_mem_ids,
                                    latest_mem_ids, fname_vector);
  }

  virtual void release_aide_info_impl() = 0;

  void init_aide_info() {
    m_wait_file_snapshot.resize(0);
    m_pending_output_files.clear();
    m_fname_vector.clear();
  }

  void release_aide_info() {
    if (m_wait_file_snapshot.size() || m_pending_output_files.size()
        || m_fname_vector.size()) {
      release_aide_info_impl();
      init_aide_info();
    }
  }
#endif /* SMARTSSD_FLUSH_MEMTABLE */
...
class Rdb_transaction_impl : public Rdb_transaction {
  ...
  */
...
#ifdef SMARTSSD_FLUSH_MEMTABLE
  /* handler interface for visible memtable flush */
  int flush_request_for_aide_impl(
      rocksdb::ColumnFamilyHandle * column_family,
      rocksdb::SequenceNumber seq_no,
      std::vector<int>& num_flush,
      std::vector<void*>& wait_cfd,
      ...
      std::vector<uint64_t>& earliest_mem_ids,
      std::vector<uint64_t>& latest_mem_ids,
      std::vector<std::vector<std::string>*>& fname_vector) override {
    rocksdb::Status s;
    std::vector<void*> wait_file_snapshot;
    std::vector<uint64_t> pending_output_files;

    /* Reqeust flushing visible memtable for aide */
    s = m_rocksdb_tx->FlushRequestForAIDE(column_family, seq_no, 
            earliest_mem_ids, latest_mem_ids, num_flush, wait_cfd, 
            wait_memid, wait_file_snapshot, pending_output_files, 
            fname_vector);

    if (s.ok()) {
      for (auto it = wait_file_snapshot.begin() ; 
          it != wait_file_snapshot.end() ; it++) {
        m_wait_file_snapshot.push_back(*it);
      }

      for (auto it = pending_output_files.begin() ; 
          it != pending_output_files.end() ; it++) {
        m_pending_output_files.push_back(*it);
      }

      wait_file_snapshot.resize(0);
      pending_output_files.clear();
      m_fname_vector.push_back(fname_vector[fname_vector.size() - 1]);
  ...
  int flush_request_for_aide_impl(
    ...
    /* Reqeust flushing visible memtable for aide */
    s = m_rocksdb_tx->FlushRequestForAIDE(column_families, visible_seq_nos,
            earliest_mem_ids, latest_mem_ids, num_flush, wait_cfd, 
            wait_memid, wait_file_snapshot, pending_output_files, 
            fname_vector);
  ...
  /* Interface for completing flush requests */
  int wait_flush_for_aide_impl(std::vector<void*>& wait_cfd,
      std::vector<uint64_t>& wait_memid,
      std::vector<uint64_t>& earliest_mem_ids,
      std::vector<uint64_t>& latest_mem_ids,
      std::vector<std::vector<std::string>*>& fname_vector) {
    ...
    /* Wait for completing flush request and get file list for aide and file
       snapshot */
    s = m_rocksdb_tx->WaitForFlushVisibleMemTables(wait_cfd, wait_memid,
            fname_vector, earliest_mem_ids, latest_mem_ids);

    for (uint64_t i = 0 ; i < fname_vector.size() ; i++) {
      m_fname_vector.push_back(fname_vector[i]);
    }
  ...
  int wait_flush_for_aide_impl(void* wait_cfd, uint64_t wait_memid,
      uint64_t earliest_mem_ids, uint64_t latest_mem_ids,
      std::vector<std::string>* fname_vector) {
    rocksdb::Status s;
  ...
  void release_aide_info_impl() override {
    /* Reqeust flushing visible memtable for aide */
    if (m_rocksdb_tx)
      m_rocksdb_tx->ReleaseAideInfo(m_wait_file_snapshot, m_fname_vector,
                                    &m_pending_output_files);
  }
#endif
...
#ifdef SMARTSSD
  auto f_encoders = m_converter->get_encoder_arr();
  for (uint i = 0; i < table_share->fields; i++) {
    table_share->field[i]->b_len = f_encoders[i].m_field_pack_length;
  }
#endif //SMARTSSD

#ifdef VERSION_INDEX
  const auto cf = m_pk_descr->get_cf();
  int nullbytes = m_converter->get_null_bytes_in_record();

#ifdef SMARTSSD_FLUSH_MEMTABLE
  table_share->null_bytes = nullbytes;
#endif /* SMARTSSD_FLUSH_MEMTABLE */
  // The current CTID supports maximum of 23 bits for null bitmap.
  assert(nullbytes < 4);

  // Set the null bytes for encoding it into version index. The only way
  // we can pass the table information (i.e., null bytes) into the RocksDB
  // storage engine is by passing it through the ColumnFamilyHandle. It's
  // kind of a hack, but for POC, we accept this method.
  cf->SetNullBytes(nullbytes);
#endif
...
#ifdef SMARTSSD
      read_opts.s3d_scan = (s3d_join_tab_cand != NULL);
      read_opts.ndp_key_size = table->s->key_info->key_length + 
        FIXED_HIDDEN_KEY_SIZE;
#endif 
...
int ha_rocksdb::rnd_init(bool scan) {
  ...
  if (scan) {
    ...
    if (s3d_join_tab_cand) {
      fprintf(stderr, "[SMARTSSD] table:%s, %d %s\n",
        table_share->table_name.str, __LINE__, __func__);
    }
#endif
...
#ifdef SMARTSSD_FLUSH_MEMTABLE
// handler function for flushing visible memtable
int ha_rocksdb::flush_visible_memtables() {
  DBUG_ENTER_FUNC();
  ...
  if (m_pk_descr == nullptr) {
    DBUG_RETURN(HA_EXIT_SUCCESS);
  }

  // get snapshot and column family
  rocksdb::SequenceNumber seq_no;
  rocksdb::ColumnFamilyHandle* cfh = m_pk_descr->get_cf();
  ...
  seq_no = tx->m_read_opts.snapshot->GetSequenceNumber();
  // flushing visible memtable for aide
  tx->flush_request_for_aide(m_pk_descr->get_cf(), seq_no, num_flush, wait_cfd,
                             wait_memid, earliest_mem_ids, latest_mem_ids,
                             fname_vector);
  tx->wait_flush_for_aide(wait_cfd, wait_memid, earliest_mem_ids, 
                          latest_mem_ids, fname_vector);
...
#if 0
      if (!rc) 
        fprintf(stderr, "NDP-TEST: table: %s key: %2x%2x%2x%2x\n", 
            table->s->table_name, key.data()[4], key.data()[5],
...
#if 0
      if (!rc) {
        fprintf(stderr, "NDP-TEST: table: %s key: %02x%02x%02x%02x%02x%02x%02x%02x\n", 
            table->s->table_name, 
            (unsigned char) key.data()[1], (unsigned char) key.data()[2], 
            (unsigned char) key.data()[3], (unsigned char) key.data()[4], 
            (unsigned char) key.data()[5], (unsigned char) key.data()[6], 
            (unsigned char) key.data()[7], (unsigned char) key.data()[8]);
      }
#endif
    }
...
#ifdef SMARTSSD
int ha_rocksdb::CalculateNDPCommandMessageLength(uint len, 
  struct S3D_JOIN_TAB* tab_list, uint32* _message_len, uint32* _files_len,
  std::vector<int>& num_flush, /* Added memtables */
  std::vector<void*>& wait_cfd, /* Wait tokens */ 
  std::vector<uint64_t>& wait_memid, /* Wait tokens */ 
  std::vector<uint64_t>& earliest_mem_ids, /* Wait tokens */
  std::vector<uint64_t>& latest_mem_ids, /* Wait tokens */
  std::vector<std::vector<std::string>*>& fname_vector, /* file name vector */
  std::vector<rocksdb::ColumnFamilyHandle*>& flush_cfh_list) {
  int err;
  uint32 i, total_len, files_len; // used for total length of 
  int cfh_idx = -1;
  struct S3D_JOIN_TAB* jt;
  Rdb_tbl_def* tbl_def;
  TABLE_SHARE	*s;
  std::string fullname;
...
#ifdef SMARTSSD_ROCKSDB_DEBUG
  fprintf(stderr, "total_len: %u\n", total_len);
#endif //SMARTSSD_ROCKSDB_DEBUG

  Rdb_transaction *tx = get_tx_from_thd(table->in_use);

  if (tx->m_read_opts.snapshot == nullptr) {
    tx->acquire_snapshot(true);
  }

  rocksdb::SequenceNumber seq_no =
      tx->m_read_opts.snapshot->GetSequenceNumber();
#ifdef SMARTSSD_ROCKSDB_DEBUG
  fprintf(stderr, "Seq No : %lu\n", seq_no);
#endif //SMARTSSD_ROCKSDB_DEBUG

  for (i = 0, jt = tab_list; i < len; i++, jt++) {
    // cf handle => sttables
    s = jt->table->s;

    if ((err = rdb_normalize_tablename(
            jt->table->s->normalized_path.str, &fullname)) != HA_EXIT_SUCCESS
        || !(tbl_def = ddl_manager.find(fullname))) {
      total_len = 0;
      files_len = 0;
      break;
    }

    jt->cfh = static_cast<rocksdb::ColumnFamilyHandleImpl*>(
      tbl_def->m_key_descr_arr[0].get()->get_cf());

    // Allocate the base 
    total_len += sizeof(NDPRelationInfoData);

    // Extend with each field offset
    total_len += (sizeof(uint16) * (s->fields > 22 ? 22 : s->fields));

    // added ghost/hidden field
    total_len += sizeof(uint16);

    // Extend with a filter operation associated with this table
    if (jt->op_type != NDP_FILTER_NONE) {
      total_len += sizeof(NDPFilter);
    }

    // Calculate the space required to store each file segment
    files_len += sizeof(NDPRelationFileData);
    auto cfh_it = find(flush_cfh_list.begin(), flush_cfh_list.end(), jt->cfh);
    if (cfh_it == flush_cfh_list.end()) {
      /* Make flush request and get added file numbers */
      cfh_idx++;
      tx->flush_request_for_aide(jt->cfh, seq_no, num_flush, wait_cfd,
          wait_memid, earliest_mem_ids, latest_mem_ids, fname_vector);
#ifdef SMARTSSD_ROCKSDB_DEBUG
      fprintf(stderr, "%s\n", jt->cfh->cfd()->GetName().c_str());
      fprintf(stderr, "%lu + %d = %d\n", fname_vector[cfh_idx]->size(), 
          num_flush[cfh_idx], (int)fname_vector[cfh_idx]->size() + num_flush[cfh_idx]);
#endif //SMARTSSD_ROCKSDB_DEBUG
      
      flush_cfh_list.push_back(jt->cfh);
    } 
    files_len += (sizeof(NDPRelationFileSegment) *
        (fname_vector[cfh_idx]->size() + num_flush[cfh_idx]));
  }
...
int ha_rocksdb::BuildNDPRelationInfo(void* message, 
    struct S3D_JOIN_TAB* jt, uint32* offset_, int num_files) {
  uint32 i, len_fields, offset = *offset_;
#ifdef SMARTSSD_ROCKSDB_DEBUG
  uint32 size = 0;
#endif //SMARTSSD_ROCKSDB_DEBUG
  TABLE_SHARE  *s = jt->table->s;
  NDPRelationInfo data = (NDPRelationInfo) (
      ((char*) message) + offset);
  ...
  sscanf(jt->cfh->cfd()->GetName().c_str(), "cf_name=cf%u", &(data->rel_id));
  len_fields = s->fields > 22 ? 22 : s->fields;
  data->num_segs = (uint32)num_files;
  data->column_info.num_att = len_fields + 1; // added hidden column

  /* number of pk parts */
  uint num_pk_parts = s->key_info[0].user_defined_key_parts;

  uint32 key_length = 0;
  for (i = 0 ; i < num_pk_parts ; i++) {
    data->column_info.attlen[i] = s->field[i]->b_len;
#ifdef SMARTSSD_ROCKSDB_DEBUG
    fprintf(stderr, "field[%d]: %u\n", i, s->field[i]->b_len);
    size += data->column_info.attlen[i];
#endif //SMARTSSD_ROCKSDB_DEBUG
    key_length += s->field[i]->b_len;
  }
  s->key_info->key_length = key_length;

  /* nullbit map len must be added to hidden column length */
  data->column_info.attlen[i] = 20 + s->null_bytes;
#ifdef SMARTSSD_ROCKSDB_DEBUG
  fprintf(stderr, "field[%d]: %u (hidden)\n", i, 20 + s->null_bytes);
  size += data->column_info.attlen[i];
#endif //SMARTSSD_ROCKSDB_DEBUG

  for (; i < len_fields; i++) {
    data->column_info.attlen[i + 1] = s->field[i]->b_len;
#ifdef SMARTSSD_ROCKSDB_DEBUG
    fprintf(stderr, "field[%d]: %u\n", i + 1, s->field[i]->b_len);
    size += data->column_info.attlen[i + 1];
#endif //SMARTSSD_ROCKSDB_DEBUG
  }
  ...
  if (data->column_info.num_att > 23) 
    data->column_info.num_att = 23;

  uint8 join_key_idx = jt->idx + (jt->idx >= num_pk_parts ? 2 : 1);
  data = (NDPRelationInfo) ((char*) data + 
      sizeof(data->column_info.attlen[0]) * (len_fields + 1)); // hidden column
  // NDP kernel build for postgres uses a field index starting with 1
  //data->column_info.hash_key_index = jt->idx + 1;
  //data->column_info.hash_key_len = s->field[jt->idx]->b_len;
  data->column_info.hash_key_index = join_key_idx;
  //data->column_info.hash_key_index = jt->idx + 1;
  data->column_info.hash_key_len = s->field[jt->idx]->b_len;

  offset += (sizeof(NDPRelationInfoData) + 
      sizeof(data->column_info.attlen[0]) * (len_fields + 1)); // hidden column

  if (jt->op_type == NDP_FILTER_NONE) {
    data->filter_info.num_filter = 0;
  } else {
    data->filter_info.num_filter = 1;

    data->filter_info.filters[0].op_type = jt->op_type;
    // NDP kernel build for postgres uses a field index starting with 1
    data->filter_info.filters[0].target_idx = jt->target_idx  + 
      (jt->target_idx >= num_pk_parts ? 2 : 1);

    if (jt->attr_type == NDP_FILTER_4B) {
      switch (s->field[jt->target_idx]->b_len) {
        case 2:
          data->filter_info.filters[0].attr_type = NDP_FILTER_2B;
          break;
        case 4:
          data->filter_info.filters[0].attr_type = NDP_FILTER_4B;
          break;
        case 8:
          data->filter_info.filters[0].attr_type = NDP_FILTER_8B;
          break;
        default:
          assert(0);
          break;
      }
      data->filter_info.filters[0].prefix_len = s->field[jt->target_idx]->b_len;
    } else {
      data->filter_info.filters[0].attr_type = jt->attr_type;
      data->filter_info.filters[0].prefix_len = jt->prefix_len;
    }

    data->filter_info.filters[0].operand = jt->operand;
...
/*
int ha_rocksdb::FlushMemtables(rocksdb::ColumnFamilyHandle* cfh, 
  std::vector<uint64_t> *wait_memid) {
  ...
  tx = get_tx_from_thd(table->in_use);
  //seq_no = tx->m_read_opts.snapshot->GetSequenceNumber();
  seq_no = UINT64_MAX;

  // flushing visible memtable for aide
  flushed = 0;
  tx->flush_request_for_aide(cfh, seq_no, num_flush, wait_cfd, &wait_memid, 
      fname_vector);

  for (int count : num_flush) {
    flushed += count;
  }
...
/* flush each pair of a dat file and a version index for each SST file
   returns a return code (int)
   */
int ha_rocksdb::FlushDataAndVersionIndex(void* messages, 
  uint32* offset_, struct S3D_JOIN_TAB *jt,
  std::vector<std::string>* fname_vector /* file name vector */,
  rocksdb::ViPrescreen& prescreen) {
  DBUG_ENTER_FUNC();
  
  int table_index, file_count;
  uint32 offset = *offset_;
  NDPRelationFile  file;
  NDPRelationFileSegment* seg;

  // get snapshot and column family
  file_count = (int)(fname_vector->size());
#ifdef SMARTSSD_ROCKSDB_DEBUG
  fprintf(stderr, "file number %lu\n", fname_vector->size());
#endif //SMARTSSD_ROCKSDB_DEBUG

  // added up with SST files
  // file_count += jt->cfh->CountFiles();

  file = (NDPRelationFile) ((char*) messages + offset);

  sscanf(jt->cfh->cfd()->GetName().c_str(), "cf_name=cf%u", &(file->id));
  file->num_segs = file_count;
  offset += ((char*) file->seg - (char*) file);

  seg = (NDPRelationFileSegment*) ((char*) messages + offset);

  // jt->cfh->GetFiles(&files);

  for (const std::string& name: *fname_vector) {
#ifdef WITH_NDP_LIB
    table_index = Get_TableIndex(es_ndpo, file->id);
#else 
    table_index = file->id;
#endif //WITH_NDP_LIB

    if (table_index == -1) {
      fprintf(stderr, "[SMARTSSD] invalid table index at %s(%d)\n", 
          __FILE__, __LINE__);
    }

#ifdef VERSION_INDEX
    // Actual prescreening of the .sst file.
    prescreen.Prepare(name);
    prescreen.Run();

    // REQUIRED: to be called before Reset()
    seg->data = prescreen.SstFile();
    seg->ctid = prescreen.IndexFile();

    if (seg->ctid == rocksdb::ViPrescreen::VI_EMPTY_FD) {
      seg->data = rocksdb::ViPrescreen::VI_EMPTY_FD;
    }

    // Keep track of open sstables. Closed on s3d_join_push().
    sst_fds.insert(seg->data);

#ifdef SMARTSSD_FLUSH_MEMTABLE
    fsync(seg->data);
#endif
    // Clear out all results of prescreening
    prescreen.Reset();
#endif

#ifdef WITH_NDP_LIB
    FlushVersionIndex(es_ndpo, table_index, 1, 
        &seg->data, &seg->ctid);
#endif //WITH_NDP_LIB

    offset += sizeof(NDPRelationFileSegment);
    seg++;
  }
  *offset_ = offset;
...
bool ha_rocksdb::s3d_join_push() {
  DBUG_ENTER_FUNC();

  DBUG_ASSERT(s3d_join_tab_cand != NULL);
  DBUG_ASSERT(s3d_first_table_cand == true);
#ifdef SMARTSSD_DEBUG
  fprintf(stderr, "[SMARTSSD] SHOULD let FPGA do join here!, %d %s\n", __LINE__, __func__);
#endif
  int err;
  uint32 i, total_len, files_len, offset; // used for total length of 
  struct S3D_JOIN_TAB *jt, *jt_list;
  NDPCommandMessage message = NULL;
  NDPRelationFile files = NULL;
  //Rdb_tbl_def* tbl_def;
  jt_list = (struct S3D_JOIN_TAB*) s3d_join_tab_cand;

  std::vector<std::string> memtables;
  /* Added memtables */
  std::vector<int> num_flush;
  /* Wait tokens */
  std::vector<void*> wait_cfd;
  ...
  /* !!! When we calculate message length, newly flushed file MUST be 
     calculated */
  CalculateNDPCommandMessageLength(s3d_len_join_tab, jt_list, 
      &total_len, &files_len, num_flush, wait_cfd, wait_memid, earliest_mem_ids,
      latest_mem_ids, fname_vector, flush_cfh_list);

  message = (NDPCommandMessage) malloc(total_len);
  message->command_type = NDP_COMMAND_JOIN;
  message->len = total_len;
  message->num_relations = s3d_len_join_tab;
  offset = (char*) message->relations - (char*) message;

  for (i = 0, err = 0, jt = jt_list; i < s3d_len_join_tab; i++, jt++) {
    cfh_idx = find(flush_cfh_list.begin(), flush_cfh_list.end(), jt->cfh) - 
        flush_cfh_list.begin();

    if ((err = BuildNDPRelationInfo(message, jt, &offset, 
         num_flush[cfh_idx] + (int)fname_vector[cfh_idx]->size()))) {
      // TODO: if NDP command could be built, we must go back to the normal routine
      fprintf(stderr, "[SMARTSSD] failed to initiate NDP Command\n");
      if (message)
        free(message);
...
#ifdef WITH_NDP_LIB
  es_fpga_mgr = FPGAManager_GetInstance(s3d_kernel_name_ptr, true);
  // Cleanup previous NDP call
  for (auto sst_fd: sst_fds) {
    rocksdb::ViUtil::Close(sst_fd);
  }
  sst_fds.clear();
  es_ndpo = NDPObject_GetInstance(es_fpga_mgr, (void *) message, "");
#endif //NDP_CALL

  if (message) 
    free(message);

  files = (NDPRelationFile) malloc(files_len);
  offset = 0; //(char*) files->seg - (char*) files;
...
#ifdef VERSION_INDEX
  const rocksdb::Snapshot* snapshot = tx->m_read_opts.snapshot;

  rocksdb::ViPrescreen prescreen(snapshot);
#endif

  bool * flushed = new bool[flush_cfh_list.size()];
  memset(flushed, 0, sizeof(bool) * flush_cfh_list.size());

  for (i = 0, err = 0, jt = jt_list; i < s3d_len_join_tab; i++, jt++) {
    jt->cfh->cfd()->es_ndpo_ = es_ndpo;
    ...
    if (!flushed[cfh_idx]) {
      cfd_idx = 0;
      for (auto cur_cfd : wait_cfd) {
        if ((uintptr_t)cur_cfd == (uintptr_t)(jt->cfh->cfd())) {
          break;
        }
        cfd_idx++;
      }
      if (wait_cfd.size() > cfd_idx) {
        tx->wait_flush_for_aide(wait_cfd[cfd_idx], wait_memid[cfd_idx],
          earliest_mem_ids[cfd_idx], latest_mem_ids[cfd_idx],
          fname_vector[cfh_idx]);
      }
      flushed[cfh_idx] = true;
    }
#ifdef VERSION_INDEX_DEBUG_SYSBENCH
    rocksdb::ViPrescreen::RunDebug(*(fname_vector[cfh_idx]), snapshot);
#endif

    FlushDataAndVersionIndex(files, &offset, jt, fname_vector[cfh_idx], 
        prescreen);

#ifdef VERSION_INDEX
    fprintf(stderr, "[ViPrescreen] Total count: %u\n", prescreen.TotalCount());
    prescreen.ResetTotalCount();
#endif
  }

#if 0 //#ifdef WITH_NDP_LIB // Write-out records in NDP
  int ret;
  size_t written;
  unsigned int blockNum;
  unsigned int id;
  char filename[64];
  char* packet = (char*) malloc(4UL << 20);
  FILE* f;

  for (i = 0, jt = jt_list; i < s3d_len_join_tab; i++, jt++) {
    //jt->cfh->SetNDPObject(es_ndpo);
    jt->cfh->cfd()->es_ndpo_ = es_ndpo;
    sscanf(jt->cfh->cfd()->GetName().c_str(), "cf_name=cf%u", &id);
    //table_index = Get_TableIndex(es_ndpo, id);
    std::sprintf(filename, "/tmp/ndp.%d", id);
    f = std::fopen(filename, "wt");

    blockNum = 0;
    do {
      while ((ret = Get_Packets(es_ndpo, id, packet, blockNum)) == -2);
      written = fwrite(packet, sizeof(char), 4UL << 20, f);
      fprintf(stderr, "Get_Packets id: %d %uth block ret: %d "
          "#recs: %lu #rec size: %lu written: %lu\n", 
          id, blockNum, ret, *((unsigned int*) packet), 
          *((unsigned short*) (packet + 4)), written);
      blockNum++;
    } while (ret > 0);
    fclose(f);
  }
...
rocksdb::Iterator *rdb_tx_get_iterator(
    ...
    const rocksdb::Slice &upper_bound_slice, 
#ifdef SMARTSSD
    uint16_t ndp_key_size,
    bool s3d_scan,
    uint s3d_buffer_size,
    uint s3d_total_buffer_read,
    uint s3d_page_size,
#endif
    bool read_current,
  ...
  return tx->get_iterator(column_family, skip_bloom_filter, fill_cache,
                          lower_bound_slice, upper_bound_slice, 
                          read_current, create_snapshot
#ifdef SMARTSSD
                          , ndp_key_size,
                          s3d_scan,
                          s3d_buffer_size,
                          s3d_total_buffer_read,
                          s3d_page_size
#endif
                         );
}
