...
class ha_rocksdb : public my_core::handler {
  ...
  */
...
#ifdef SMARTSSD
  int BuildNDPRelationInfo(void* message, 
      struct S3D_JOIN_TAB* jt, uint32* offset_, int num_files);
  int CalculateNDPCommandMessageLength(uint len, struct S3D_JOIN_TAB* tab_list, 
  ...
  int FlushMemtables(rocksdb::ColumnFamilyHandle* cfh, 
    std::vector<uint64_t> *wait_memid);
  int FlushDataAndVersionIndex(void* messages, 
      uint32* offset_, struct S3D_JOIN_TAB *jt,
      std::vector<std::string>* fname_vector, rocksdb::ViPrescreen& prescreen);
  bool s3d_join_push() override;
#endif
